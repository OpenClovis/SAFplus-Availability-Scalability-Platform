...
THIRDPARTY_NAME_STARTS_WITH  = '3rdparty-base-1.20'                # Look for PKG starting with this name
SUPPORT_EMAIL                = 'matt@openclovis.com'               # email for script maintainer
INSTALL_LOCKFILE             = '/tmp/.openclovis_installer'        # installer lockfile location
THIRDPARTYPKG_DEFAULT        = '3rdparty-base-1.20.tar'            # search this package if no 3rdPartyPkg found
...
#                                     ret_code = cli_cmd(cmd)
    ...
    def launchGUI(self):
        ...
        if not self.PREINSTALL_ONLY:
            ...
            for DIR in (self.INSTALL_DIR, self.BUILDTOOLS, self.PACKAGE_ROOT, self.PREFIX, self.PREFIX_BIN, self.PREFIX_LIB, self.IDE_ROOT, 
                        ...
                        self.CACHE_DIR, self.ECLIPSE, self.ECLIPSE_ROOT, self.ESC_ECLIPSE_DIR):
        ...
        ThirdPartyList = os.popen ("ls %s.*tar" % THIRDPARTY_NAME_STARTS_WITH)
        ThirdPartyList = ThirdPartyList.readlines()
        ...
        if len (ThirdPartyList) == 0:
           ...
           Pkg_Found = 1
           for THIRDPARTYPKG  in ThirdPartyList:
               pass
        else:
           Pkg_Found = 1
    ...
    def install_IDE(self):
        ...
        NOW_VERSION = '%s/src/ASP/VERSION' % self.PACKAGE_ROOT
        ...
        if os.path.isfile(VERSION_CACHE):
            ...
            try:
                ...
                if do_nothing:
                    ...
                    cmds = ['cp $PACKAGE_ROOT/src/ASP/VERSION $CACHE_DIR/eclipse/.',
    ...
    def prebuild(self):
        ...
        if not strin or strin.lower().startswith('y'):
           ...
           os.chdir (cmd)
           self.feedback ('Select the crossbuild tool(s) to build from the above list, [Default: none]') 
           ...
           if strin == 'local':
              ...
              syscall ("%s/src/ASP/configure --with-asp-build --with-cross-build=%s > build.log" % (self.PACKAGE_ROOT, strin) )
           
           self.feedback('Building asp %s' % strin)
           cmd = 'asp/build/%s' % strin
           os.chdir (cmd)
    ...
    def do_symlinks(self):
        ...
        self.feedback('A few binaries are installed in %s\n' % self.BIN_ROOT)
        ...
        os.chdir (self.DEFAULT_SYM_LINK)

        files = syscall ("ls %s | egrep -v 'cl-log-viewer|config|icons'" % self.BIN_ROOT) 

        for file in files:
           os.system ('ln -s %s/%s' % (self.BIN_ROOT, file))
    ...
    def parse_unix_vars(self, line):
        ...
        self.ESC_PKG_NAME = syscall("echo %s | sed -e 's/\./\\\./g'" % self.PACKAGE_NAME)
        #self.ESC_ECLIPSE_DIR = syscall("echo %s/eclipse | sed -e 's;/;\\\/;g'" % self.PACKAGE_ROOT)
        olist = ['PREFIX', 'THIRDPARTYPKG', 'BUILDTOOLS', 'NET_SNMP_CONFIG', 'PACKAGE_ROOT', 'BIN_ROOT', 'LIB_ROOT', 'WORKING_DIR', 'ESC_PKG_ROOT', 'ESC_PKG_NAME', 'IDE', 'ASP', 'PACKAGE_NAME', 'HOME', 'CACHE_DIR', 'IDE_ROOT', 'ECLIPSE_ROOT', 'ECLIPSE', 'ESC_ECLIPSE_DIR']
        rlist = [self.PREFIX, self.THIRDPARTYPKG_PATH, self.BUILDTOOLS, self.NET_SNMP_CONFIG, self.PACKAGE_ROOT, self.BIN_ROOT, self.LIB_ROOT, self.WORKING_DIR, self.ESC_PKG_ROOT, self.ESC_PKG_NAME, 'IDE', 'ASP', self.PACKAGE_NAME, self.HOME, self.CACHE_DIR, self.IDE_ROOT, self.ECLIPSE_ROOT, self.ECLIPSE, self.ESC_ECLIPSE_DIR]
