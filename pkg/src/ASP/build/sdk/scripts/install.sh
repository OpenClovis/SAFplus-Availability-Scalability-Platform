...
#If First ASP SDK installation , create new directories
if [ ! -f $CONF ]; then
    ...
    for d in `find $CLOVIS_ROOT/ASP/components -type d -name include`; do
    ...
    cp -pr $CLOVIS_ROOT/ASP/components/include/ipi $SDK_INCLUDE
    cp -pr $CLOVIS_ROOT/ASP/components/cor/common/clCorTxnClientIpi.h $SDK_INCLUDE 
