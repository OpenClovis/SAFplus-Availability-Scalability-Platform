...
def start_amf():
    ...
    if is_valgrind_build() and is_amf_in_valgrind_list():
        cmd = '%s; ' % get_asp_ulimit_cmd() +\
              ...
              '--log-file=%s/asp_amf.%d ' % (get_asp_log_dir(), int(time.time())) +\
              '%s/asp_amf -c 0 -l %s -n %s' %\
...
def kill_asp(lock_remove = True):
    ...
    # Insert asp_amf at the top of the list for kills so there is no false recovery from amf which could also
    # result in a node failfast resulting in node reboots
    try:
        apps.remove('asp_amf')
        apps.insert(0, 'asp_amf')
    except: pass
