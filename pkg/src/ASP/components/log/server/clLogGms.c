...
static ClHandleT gTipcCallbackHandle = CL_HANDLE_INVALID_VALUE;
static void 
...
static void
clLogTipcNodedownCallback(ClIocNotificationIdT eventId, 
                           ClPtrT               pArg,
                           ClIocAddressT        *pAddress)
{
    ...
    if( CL_OK != rc )
    {
        clLogError("SVR", "TIPC", "Failed to get svr common data rc[0x %x]",
                rc);
        return ;
    }    
...
clLogCpmNotificationCallbacksUpdate(ClIocNodeAddressT leader)
{
    ...
    if(CL_HANDLE_INVALID_VALUE != gTipcCallbackHandle)
    {
        clCpmNotificationCallbackUninstall(&gTipcCallbackHandle);
    }
    ...
    if( CL_HANDLE_INVALID_VALUE != gTipcCallbackHandle )
    {
        clCpmNotificationCallbackInstall(compAddr, clLogTipcNodedownCallback, 
                NULL, &gTipcCallbackHandle);
    }
    return rc;
}
                ...
                ClIocNodeAddressT  deputy)
{
    ...
    if( (pSvrCommonEoData->deputyAddr == leader) && (localAddr == leader) )/*switch over*/
    {
        pSvrCommonEoData->masterAddr = leader;
        pSvrCommonEoData->deputyAddr = deputy;
        /*
