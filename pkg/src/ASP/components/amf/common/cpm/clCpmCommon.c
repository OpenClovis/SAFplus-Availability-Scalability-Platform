...
ClRcT cpmPrintDBXML(FILE *fp)
{
    ClRcT rc = CL_OK;
    ClCpmLT *cpmL = NULL;
    ClGmsNodeIdT *nodeIds[] = { &gpClCpm->activeMasterNodeId,
                                &gpClCpm->deputyNodeId,
    };
    register ClInt32T i;
    ...
    for(i = 0; i < sizeof(nodeIds)/sizeof(nodeIds[0]);++i)
    {
        ...
        {
            rc = cpmNodeFindByNodeId(*nodeIds[i],&cpmL);
            if(rc != CL_OK)
            {
            ...
            fprintf(fp,"<ha_state value=\"%s\"/>\n",nodeIds[i] == &gpClCpm->activeMasterNodeId ? "active" : "standby");
            fprintf(fp,"</node>\n");
        }
    }
    rc = CL_OK;
