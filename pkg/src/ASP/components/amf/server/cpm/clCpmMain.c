...
ClRcT cpmEventInitialize(void)
{
    ...
    if (rc != CL_EVENT_ERR_ALREADY_INITIALIZED)
        CL_CPM_CHECK_2(CL_DEBUG_ERROR, CL_LOG_MESSAGE_2_LIBRARY_INIT_FAILED,
    ...
    if (rc != CL_EVENT_ERR_EXIST)
        CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_EVT_CHANNEL_OPEN_ERR, rc,
    ...
    */
    ...
    CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_EVT_CHANNEL_ALLOC_ERR, rc, rc,
                                  ...
                                  ClCpmNodeEventT operation)
{
    ...
    CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_BUF_FLATTEN_ERR, rc, rc,
            CL_LOG_DEBUG, CL_LOG_HANDLE_APP);
...
static void cpmSigchldHandler(ClInt32T signum)
{
    ...
    /* 
     * Added the do-while loop. 
     * (With Bug 4028, in addition to bugfix.)
    ...
    else
    {
      ...
      mallocFailed:
        ...
        if(rc != CL_OK)
        {
            ...
            CL_DEBUG_PRINT(CL_DEBUG_ERROR, ("Eo list mutex lock returned [%#x]", rc));
            goto failure;
        }
...
static ClRcT cpmCreateLoggerTaskPipe(const ClCharT *cpmLoggerFile)
{
    ...
    if(cpmLoggerFd < 0)
    {
        ...
        CL_DEBUG_PRINT(CL_DEBUG_ERROR, ("CPM logger file [%s] open returned [%s]\n",
    ...
    {
        CL_DEBUG_PRINT(CL_DEBUG_ERROR, ("CPM logger pipe create returned [%s]\n",
                                        strerror(errno)));
        goto out_close;
    }
...
static ClRcT clCpmInitialize(ClUint32T argc, ClCharT *argv[])
{
     ...
     */
    rc = cpmAllocate();
    CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_SERVER_CPM_ALLOCATE_ERR, rc, rc,
                   CL_LOG_DEBUG, CL_LOG_HANDLE_APP);
    ...
    CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_EO_OBJECT_ERR, rc, rc,
                   CL_LOG_DEBUG, CL_LOG_HANDLE_APP);
    ...
    /*
     * Install EO protocol to get IOC/TIPC notifications for component
    ...
    rc = cpmDebugRegister();
    CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_DEBUG_REG_ERR, rc, rc,
                   CL_LOG_ERROR, CL_LOG_HANDLE_APP);
    ...
    if (gpClCpm->pCpmConfig->cpmType == CL_CPM_GLOBAL)
    {
        ...
        rc = clAmsInitialize(&gAms, gpClCpm->amsToCpmCallback,
                             gpClCpm->cpmToAmsCallback, CL_AMS_INSTANTIATE_MODE_ACTIVE);
        CL_CPM_CHECK(CL_DEBUG_ERROR, ("Unable to initialize AMS %x\n", rc), rc);
        */
    }
#endif
#ifndef CL_CPM_GMS
                               ...
                               ClBufferHandleT outMsgHdl)
{
    ...
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Inside compMgrEORegister \n"));
    ...
    if (info.version > CL_CPM_EO_VERSION_NO)
        CL_CPM_CHECK_0(CL_DEBUG_ERROR, CL_LOG_MESSAGE_0_VERSION_MISMATCH,
...
static ClRcT _compMgrFuncEOWalk(ClCpmFuncWalkT *pWalk)
{
    ...
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Inside compMgrFuncEOWalk \n"));
    if (pWalk == NULL)
                                  ...
                                  ClBufferHandleT outMsgHandle)
{
    ...
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Inside compMgrEOStateUpdate \n"));
                               ...
                               ClBufferHandleT outMsgHandle)
{
    ...
    if (eoId == 0xffff)         /* This means all the EOs */
    {
        ...
        if (rc != CL_OK)
            CL_DEBUG_PRINT(CL_DEBUG_ERROR, ("COMP_MGR : EoWalk failed.\n"));
    }
    else
    {
        /*
        ...
        if (found == 1)
        {
            if (isDead == 0)
            {
                ...
                CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_RMD_CALL_ERR, rc,
                               rc, CL_LOG_DEBUG, CL_LOG_HANDLE_APP);
            }
        }
        else
        {
            CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_SERVER_EO_UNREACHABLE,
                           rc, rc, CL_LOG_DEBUG, CL_LOG_HANDLE_APP);
        }
    }
...
void cpmRestart(ClTimerTimeOutT *pDelay, const ClCharT *pPersonality)
{
     ...
     * a re-establish, TIPC reestablish takes some time 
     * and the other guy would miss the TIPC node leave notification packet to be
     * sent by the kernel TIPC topology service to the subscribers, incase
     * we die at/before TIPC link re-establishment happens.
     */
                        ...
                        ClGmsNodeIdT deputyNodeId)
{
    ...
    if (CL_OK != rc)
    {
        ...
        clLogCritical(CPM_LOG_AREA_CPM, CPM_LOG_CTX_CPM_AMS,
                      ...
                      "which has been mostly seen to be caused by openais/tipc config. issues."
...
ClRcT cpmEoPtrFind(ClEoIdT eoID, ClCpmEOListNodeT **pThis)
{
    ...
    while (compCount && rc == CL_OK)
    {
        ...
        {
            if ((rc = clOsalMutexLock(gpClCpm->eoListMutex)) != CL_OK)
                CL_CPM_CHECK_1(CL_DEBUG_ERROR, CL_CPM_LOG_1_OSAL_MUTEX_LOCK_ERR,
                        rc, rc, CL_LOG_DEBUG, CL_LOG_HANDLE_APP);
            ptr = comp->eoHandle;
            while (ptr != NULL)
            {
                           ...
                           ClBufferHandleT recvMsg)
{
    ...
    if (rc == CL_OK)
    {
        ...
        else if (retCode == CL_OK)
        {
            rc = VDECL_VER(clXdrUnmarshallClEoSchedFeedBackT, 4, 0, 0)(recvMsg, (void *) &outBuffer);
            if (rc != CL_OK)
                CL_CPM_LOCK_CHECK(CL_DEBUG_ERROR, ("Invalid Buffer Passed \n"),
...
static ClRcT clCpmIocNotificationHandler(ClPtrT invocation)
{
    ...
    if(protoVersion != CL_IOC_NOTIFICATION_VERSION)
    {
        clLogError(CPM_LOG_AREA_CPM, CPM_LOG_CTX_CPM_AMS,
                   "CPM got tipc notification packet with version [%d]. "
    ...
    switch(notificationId)
    {
                ...
                (iocPort == CL_IOC_CPM_PORT))
            {
                ...
                if (CL_CPM_IS_ACTIVE())
                {
                    clLogMultiline(CL_LOG_CRITICAL,
                                   ...
                                   CPM_LOG_CTX_CPM_AMS,
                                   "CPM/G active got "
                                   "IOC/TIPC notification for node [%d] -- \n"
        ...
        default:
        {
            clLogError(CPM_LOG_AREA_CPM, CL_LOG_CONTEXT_UNSPECIFIED,
                       "CPM got IOC/TIPC notification with "
...
static ClRcT clCpmIocNotificationEnqueue(ClIocNotificationT *notification, ClPtrT cookie)
{
    ...
    if(rc != CL_OK)
    {
        clLogError("IOC", "NOT", "CPM IOC/TIPC notification enqueue returned with [%#x]", rc);
        clHeapFree(job);
    }
    return rc;
}
...
/*
 * Unused now because of the above fast interface through tipc
 */
...
ClRcT compMgrPollThread(void)
{
    ...
    while (gpClCpm->polling && gpClCpm->enableHeartbeat == CL_TRUE)
    {
        ...
        if (rc != CL_OK)
        {
            ...
            CL_DEBUG_PRINT(CL_DEBUG_ERROR, ("Get First component %x\n", rc));
        }
        ...
        {
            /*
             * CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Inside Polling while loop for
             * local components %d\n", compCount));
             */
            rc = clCntNodeUserDataGet(gpClCpm->compTable, hNode,
                                      (ClCntDataHandleT *) &comp);
