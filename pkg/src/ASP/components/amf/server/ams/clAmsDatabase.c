...
ClRcT
clAmsAuditDbEpilogue(ClNameT *nodeName)
{
    if(!nodeName) return CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
    clLogDebug("AUDIT", "DB", "Running AMF audit db prologue functions for node [%s]", nodeName->value);
    return cpmReplayInvocations(CL_TRUE);
}
