                                  ...
                                  ClIocAddressT *destAddress, ClBoolT compat)
{
    ...
    retCode = clIocSend(commPort, message, CL_IOC_PORT_NOTIFICATION_PROTO, destAddress, &sendOption);
    if(retCode != CL_OK)
...
/*
 * Send the node bitmap for this send on receiving node arrival.
 */
static ClRcT clIocNotificationNodeMapSend(ClIocCommPortHandleT commPort, ClIocAddressT *compAddress)
{
    ...
    rc = clIocSend(commPort, message, CL_IOC_PROTO_CTL, compAddress, &sendOption);
...
static ClRcT clIocNodeVersionSend(ClIocCommPortHandleT commPort, ClIocAddressT *destAddress)
{
    ...
    return clIocNotificationPacketSend(commPort, &notification, destAddress, CL_FALSE);
}
/* 
                                      ...
                                      ClIocAddressT *allLocalComps, ClIocAddressT *allNodeReps)
{
    ...
    if(notificationNodeAddr == gIocLocalBladeAddress)
    {
        ...
        else 
        {
            ...
            rc = clIocNodeVersionSend(commPort, allNodeReps);
        }
        return rc;
    }
    ...
    if(status == CL_IOC_NODE_UP)
    {
        ...
        */
        clIocNodeVersionSend(commPort, (ClIocAddressT*)&notificationCompAddr);
    ...
    else 
    {
        ...
        clTipcMasterSegmentUpdate(notificationCompAddr);
    ...
    return clIocNotificationPacketSend(commPort, &notification, allLocalComps, CL_FALSE);
}
                                      ...
                                      ClIocAddressT *allLocalComps, ClIocAddressT *allNodeReps)
{
    ...
    return clIocNotificationPacketSend(commPort, &notification, allNodeReps, CL_FALSE);
}         
                                  ...
                                  ClIocAddressT *allLocalComps, ClIocAddressT *allNodeReps)
{
       ...
       ntohl(notification.id) == CL_IOC_NODE_VERSION_REPLY_NOTIFICATION)
    {
        ...
        if(ntohl(notification.id) == CL_IOC_NODE_VERSION_NOTIFICATION)
        {
            ...
            notification.nodeAddress.iocPhyAddress.nodeAddress = htonl(gIocLocalBladeAddress);
            notification.nodeAddress.iocPhyAddress.portId = htonl(myCapability);
            rc = clIocNotificationPacketSend(commPort, &notification, &destAddress, compat);
        }
        goto out;
    }
    ...
    clLogInfo ("TIPC", "NOTIF", "Got [%s] notification for node [0x%x] commport [0x%x]",
               event == CL_IOC_NODE_UP ? "arrival": "death", compAddr.nodeAddress, compAddr.portId); 
