...
static ClRcT clEoStart(ClEoExecutionObjT *pThis)
{
     ...
     * level Queue
     */
    rc = clEoTaskCreate("eoStaticQueueProcess", (ClEoTaskFuncPtrT) eoStaticQueueProcess, pThis, &gClEoTaskIds[gClEoThreadCount++]);
    if (CL_OK != rc)
    {
        goto eoStaticQueueInitialized;
    }
     ...
     */
    rc = clEoTaskCreate("clEoIocRecvQueueProcess", (ClEoTaskFuncPtrT) clEoIocRecvQueueProcess, pThis, &gClEoTaskIds[gClEoThreadCount++]);
    if (CL_OK != rc)
    {
        goto eoStaticQueueInitialized;
    }
...
ClRcT clEoUnblock(ClEoExecutionObjT *pThis)
{
    ...
    for(i = 0; i < gClEoThreadCount-1; ++i)
    {
        if(gClEoTaskIds[i])
        {
