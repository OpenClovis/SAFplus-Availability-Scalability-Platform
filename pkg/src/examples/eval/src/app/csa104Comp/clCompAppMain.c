...
extern ClRcT clcsa104CompEO0ClientInstall(void);
...
ClCharT appname[80];               // component instance name
...
#define CKPT_NAME     "csa104Ckpt"  // checkpoint name for this app
    ...
    ClCharT *argv[])
{
    ...
    rc = clcsa104CompEO0ClientInstall();
...
#define min(x,y) ((x<y)? x: y)
    ...
    clprintf(CL_LOG_SEV_INFO,"csa104: instantiated as component instance %s.", appname);
...
#if defined(CL_INST)
if ((rc = clDataTapInit(DATA_TAP_DEFAULT_FLAGS, 104)) != CL_OK)
    {
    ...
    ClAmsCSIDescriptorT csiDescriptor)
{
    ...
    switch ( haState )
    {
        case CL_AMS_HA_STATE_ACTIVE:
        {
            ...
            clprintf(CL_LOG_SEV_INFO,"%s: HA State assigned - ACTIVE ", appname);
            ...
            if (CL_OK != rc)
            {
                clprintf(CL_LOG_SEV_ERROR,"%s: Failed [0x%x] to make the OI as a primary OI. ",
                        appname, rc);
                return rc;
            }
        ...
        case CL_AMS_HA_STATE_STANDBY:
        {
            ...
            clprintf(CL_LOG_SEV_INFO,"%s: HA State assigned - STANDBY ", appname);
        ...
        case CL_AMS_HA_STATE_QUIESCED:
        {
            ...
            clprintf(CL_LOG_SEV_INFO,"%s: HA State assigned - QUIESCED ", appname);
            ha_state = haState;

            rc = clCorPrimaryOIClear(&moId, &addr);
            if (CL_OK != rc)
            {
                clprintf(CL_LOG_SEV_ERROR,"%s: Failed [0x%x] to clear the primary OI status. ",
                       appname, rc);
                return rc;
            }
...
update_counter_in_cor(time_t now, ClCorObjectHandleT objH, ClUint32T counter)
{
    ...
    // should go here
        rc = clCorObjectAttributeSet(CL_COR_SIMPLE_TXN, objH, NULL, CSA104RES_COUNTER, 
                CL_COR_INVALID_ATTR_IDX, &counter, sizeof(counter));
        if (CL_OK != rc)
        {
            clprintf(CL_LOG_SEV_INFO,"%s: Failed [0x%x] to set the value of counter variable.",
                   appname, rc);
            return rc;
        }
