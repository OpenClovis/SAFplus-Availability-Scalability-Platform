...
extern ClRcT csa203Comp_0EO0ClientInstall(void);
...
static char           appname[80];  /* Component instance name               */
static ClUint32T      seq      = 0; /* Sequence number for print lines       */
static SaAmfHAStateT  ha_state =  0; /* HA state           */

#define CKPT_NAME     "csa203Ckpt"  /* Checkpoint name for this application  */
...
int main(int argc, char *argv[])
{
    ...
    rc = csa103Comp_0EO0ClientInstall();
    ...
    clprintf(CL_LOG_SEV_INFO,"csa203: Initializing and registering with CPM...\n");
    ...
    strcpy(appname, (ClCharT *)appName.value);


    clprintf(CL_LOG_SEV_INFO,"csa203: Instantiated as component instance %s.\n", appName.value);
    ...
    {
        clprintf(CL_LOG_SEV_INFO,"%s: Failed [0x%x] to initialize checkpoint\n",
                    appname, rc);
        return rc;
    }
    if ((rc = checkpoint_read_seq(&seq)) != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"%s: Failed [0x%x] to read checkpoint\n",
                    appname, rc);
        checkpoint_finalize();
        return rc;
    }
    ...
    {
        clprintf(CL_LOG_SEV_INFO,"%s: Failed [0x%x] to initialize data tap\n",
                    appname, rc);
    }
#endif
    while (!exiting)
    {
        if (running && ha_state == SA_AMF_HA_ACTIVE)
        {
            ...
            {
                clprintf(CL_LOG_SEV_INFO,"%s: Failed [0x%x] to send data tap data\n",
                            appname, rc);
            }
#endif
            ...
            if (rc != SA_AIS_OK)
            {
                clprintf(CL_LOG_SEV_INFO,"%s: ERROR: Checkpoint write failed. Exiting.\n", appname);
                break;
            }
        }
        sleep(1);
    ...
    // ...
    clprintf(CL_LOG_SEV_INFO,"csa203Comp: Finalize called.\n");
    checkpoint_finalize();
                        ...
                        SaAmfCSIDescriptorT csiDescriptor)
{
    ...
    switch ( haState )
    {
        ...
        case SA_AMF_HA_QUIESCING:
        {
            ...
            // ...
            clprintf(CL_LOG_SEV_INFO,"%s: QUIESCING\n", appname);
            ha_state = haState;
...
checkpoint_initialize()
{
    ...
    int retry_count = 0;
    const int max_create_retry = 2;
    int need_create = 0;                // do we need to create the section?
    ...
    if (rc != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"%s: ERROR: Failed to initialize checkpoint service\n",
                    appname);
        return rc;
    }
    ...
    if (rc == SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"%s: CheckpointOpen with CREATE succeeded\n",
                        appname);
        break;
    }
    need_create = 1;
    rc = saCkptCheckpointOpen(ckpt_svc_handle,      // Service handle
                                ...
                                (SaTimeT)-1,        // No timeout
                                &ckpt_handle);      // Checkpoint handle
    } while ( (rc != SA_AIS_OK) && (++retry_count < max_create_retry));

    if (rc != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"%s: ERROR: Failed [0x%x] to open checkpoint\n",
                    appname, rc);
        (void)clCkptFinalize(ckpt_svc_handle);
        return rc;
    }
    clprintf(CL_LOG_SEV_INFO,"%s: Checkpoint opened (handle=0x%llx)\n", appname, ckpt_handle);
    ...
    if (need_create)
    {
        ...
        {
            clprintf(CL_LOG_SEV_INFO,"%s: ERROR: Failed to create checkpoint section\n", appname);
        ...
        else if (rc != SA_AIS_OK && rc == SA_AIS_ERR_EXIST)
        {
            ...
            if (rc != SA_AIS_OK)
            {
                clprintf(CL_LOG_SEV_INFO,"%s: ERROR: Failed [0x%x] to read checkpoint section\n",
                            appname, rc);
    ...
    else
    {
        ...
        if (rc != SA_AIS_OK)
        {
            clprintf(CL_LOG_SEV_INFO,"%s: ERROR: Failed [0x%x] to read Ckpt section\n",
                        appname, rc);
            (void)clCkptCheckpointClose(ckpt_handle);
            (void)clCkptFinalize(ckpt_svc_handle);
        }
...
checkpoint_finalize(void)
{
    ...
    if (rc != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"%s: failed: [0x%x] to close checkpoint handle %llx\n",
                    appname, rc, ckpt_handle);
    }
    rc = saCkptFinalize(ckpt_svc_handle);
    if (rc != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"%s: failed: [0x%x] to finalize checkpoint\n",
                    appname, rc);
    }
    return SA_AIS_OK;
}
...
checkpoint_write_seq(ClUint32T seq)
{
    ...
    seq_no = htonl(seq);
//    clprintf(CL_LOG_SEV_INFO,"checkpoint_write_seq: seq_no = %lu, seq = %lu\n", seq_no, seq); fflush(stdout);
    ...
    if (rc != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_INFO,"Failed [0x%x] to write to section\n", rc);
    }
    else
    {
        ...
        if (rc != SA_AIS_OK)
        {
            clprintf(CL_LOG_SEV_INFO,"Failed [0x%x] to synchronize the checkpoint\n", rc);
        }
    }
...
checkpoint_read_seq(ClUint32T *seq)
{
    ...
    if (rc != SA_AIS_OK )
    {
        clprintf(CL_LOG_SEV_INFO,"Error: [0x%x] from checkpoint read, err_idx = %u\n",
                    rc, err_idx);
    }

    /* FIXME: How to process this err_idx? */
    *seq = ntohl(seq_no);
//    clprintf(CL_LOG_SEV_INFO,"checkpoint_read_seq: seq_no = %lu, seq = %lu\n", seq_no, *seq); fflush(stdout);
