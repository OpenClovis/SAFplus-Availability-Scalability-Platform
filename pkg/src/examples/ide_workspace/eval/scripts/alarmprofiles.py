					...
					if len(alarmids) > 0:
						...
						for alarmid in alarmids:
							...
							if len(genRule) > 0 and len(genRule[0].getElementsByTagName("alarmIDs")) > 0:
								alarmRulesTables += createGenRuleTable(genRule[0], resName + alarmProfileName + specificProblem, alarmMap)
							...
							if len(suppRule) > 0 and len(suppRule[0].getElementsByTagName("alarmIDs")) > 0:
								alarmRulesTables += createSuppRuleTable(suppRule[0], resName + alarmProfileName + specificProblem, alarmMap)
