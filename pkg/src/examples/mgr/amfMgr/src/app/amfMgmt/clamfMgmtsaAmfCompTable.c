...
ClRcT clamfMgmtSAAMFCOMPTABLEProvConstructor( void *pThis, void *pUsrData, ClUint32T usrDataLen )
{
    ...
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectStart = clamfMgmtSAAMFCOMPTABLEProvObjectStart;      
    ((CL_OM_PROV_CLASS*)pThis)->clProvValidate = (fp)clamfMgmtSAAMFCOMPTABLEProvValidate;    
    ((CL_OM_PROV_CLASS*)pThis)->clProvUpdate = (fp)clamfMgmtSAAMFCOMPTABLEProvUpdate;    
    ((CL_OM_PROV_CLASS*)pThis)->clProvRollback = (fp)clamfMgmtSAAMFCOMPTABLEProvRollback;
    ((CL_OM_PROV_CLASS*)pThis)->clProvRead = (fp)clamfMgmtSAAMFCOMPTABLEProvRead;
	((CL_OM_PROV_CLASS*)pThis)->clProvObjectValidate = clamfMgmtSAAMFCOMPTABLEProvObjectValidate; 	 
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectUpdate = clamfMgmtSAAMFCOMPTABLEProvObjectUpdate; 	 
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectRollback = clamfMgmtSAAMFCOMPTABLEProvObjectRollback;
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectRead = clamfMgmtSAAMFCOMPTABLEProvObjectRead;
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectEnd = clamfMgmtSAAMFCOMPTABLEProvObjectEnd;      
 ...
 * clamfMgmtSAAMFCOMPTABLEOAMPConfig.h for detailed documentation for this function.
 */
...
ClRcT clamfMgmtSAAMFCOMPTABLEProvRead(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnData)
{
    ...
    ClAmsCompStatusT *pCompStatus = NULL;
    if(!gClAmsMgmtHandle)
    ...
    rc = clAmsMgmtEntityGetStatus(gClAmsMgmtHandle, &entity, 
            (ClAmsEntityStatusT**)&pCompStatus);
    if(rc != CL_OK)
    {
        clLogError("OI", "READ", "Comp [%s] status returned [%#x]", 
                entity.name.value, rc);
        return rc;
    }
    switch(pProvTxnData->attrId)
    {
        case SAAMFCOMPTABLE_SAAMFCOMPNUMCURRACTIVECSI:
            {
                if(pProvTxnData->size != (ClUint32T)sizeof(pCompStatus->numActiveCSIs))
                {
                    clLogError("OI", "READ", "Read size [%d] doesnt match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pCompStatus->numActiveCSIs));
                    rc = CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                    goto out_free;
                }
                memcpy(pProvTxnData->pProvData, &pCompStatus->numActiveCSIs, pProvTxnData->size);
            }
            break;

        case SAAMFCOMPTABLE_SAAMFCOMPNUMCURRSTANDBYCSI:
            {
                if(pProvTxnData->size != (ClUint32T)sizeof(pCompStatus->numStandbyCSIs))
                {
                    clLogError("OI", "READ", "Read size [%d] does not match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pCompStatus->numStandbyCSIs));
                    rc = CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                    goto out_free;
                }
                memcpy(pProvTxnData->pProvData, &pCompStatus->numStandbyCSIs, pProvTxnData->size);
            }
            break;

        case SAAMFCOMPTABLE_SAAMFCOMPRESTARTCOUNT:
            {
                if(pProvTxnData->size != (ClUint32T)sizeof(pCompStatus->restartCount))
                {
                    rc = CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                    clLogError("OI", "READ", "Read size [%d] doesnt match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pCompStatus->restartCount));
                    goto out_free;
                }
                memcpy(pProvTxnData->pProvData, &pCompStatus->restartCount, pProvTxnData->size);
            }
            break;

        case SAAMFCOMPTABLE_SAAMFCOMPOPERSTATE:
            {
                if(pProvTxnData->size != (ClUint32T)sizeof(pCompStatus->operState))
                {
                    rc = CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                    clLogError("OI", "READ", "Read size [%d] doesnt match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pCompStatus->operState));
                    goto out_free;
                }
                memcpy(pProvTxnData->pProvData, &pCompStatus->operState, pProvTxnData->size);
            }
            break;

        case SAAMFCOMPTABLE_SAAMFCOMPPRESENCESTATE:
            {
                if(pProvTxnData->size != (ClUint32T)sizeof(pCompStatus->presenceState))
                {
                    rc = CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                    clLogError("OI", "READ", "Read size [%d] doesnt match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pCompStatus->presenceState));
                    goto out_free;
                }
                memcpy(pProvTxnData->pProvData, &pCompStatus->presenceState, pProvTxnData->size);
            }
            break;

        case SAAMFCOMPTABLE_SAAMFCOMPREADINESSSTATE:
            {
                if(pProvTxnData->size != (ClUint32T)sizeof(pCompStatus->readinessState))
                {
                    clLogError("OI", "READ", "Read size [%d] doesnt match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pCompStatus->readinessState));
                    rc = CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                    goto out_free;
                }
                memcpy(pProvTxnData->pProvData, &pCompStatus->readinessState, pProvTxnData->size);
            }
            break;
 ...
 * Please refer protptype of this function in the file clamfMgmtSAAMFCOMPTABLEOAMPConfig.h
...
ClRcT clamfMgmtSAAMFCOMPTABLEProvObjectRead(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnDataList, ClUint32T txnDataEntries)
{
    ...
    for(i = 0; i < txnDataEntries; ++i)
    {
        rc = clamfMgmtSAAMFCOMPTABLEProvRead(pThis, txnHandle, pProvTxnDataList + i);
        if(rc != CL_OK)
            return rc;
    }
	/*
