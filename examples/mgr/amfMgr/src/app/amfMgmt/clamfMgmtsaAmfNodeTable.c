...
ClRcT clamfMgmtSAAMFNODETABLEProvConstructor( void *pThis, void *pUsrData, ClUint32T usrDataLen )
{
    ...
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectStart = clamfMgmtSAAMFNODETABLEProvObjectStart;      
    ((CL_OM_PROV_CLASS*)pThis)->clProvValidate = (fp)clamfMgmtSAAMFNODETABLEProvValidate;    
    ((CL_OM_PROV_CLASS*)pThis)->clProvUpdate = (fp)clamfMgmtSAAMFNODETABLEProvUpdate;    
    ((CL_OM_PROV_CLASS*)pThis)->clProvRollback = (fp)clamfMgmtSAAMFNODETABLEProvRollback;
    ((CL_OM_PROV_CLASS*)pThis)->clProvRead = (fp)clamfMgmtSAAMFNODETABLEProvRead;
	((CL_OM_PROV_CLASS*)pThis)->clProvObjectValidate = clamfMgmtSAAMFNODETABLEProvObjectValidate; 	 
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectUpdate = clamfMgmtSAAMFNODETABLEProvObjectUpdate; 	 
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectRollback = clamfMgmtSAAMFNODETABLEProvObjectRollback;
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectRead = clamfMgmtSAAMFNODETABLEProvObjectRead;
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectEnd = clamfMgmtSAAMFNODETABLEProvObjectEnd;      
 ...
 * clamfMgmtSAAMFNODETABLEOAMPConfig.h for detailed documentation for this function.
 */
...
ClRcT clamfMgmtSAAMFNODETABLEProvUpdate(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnData)
{
    ...
    switch (pProvTxnData->provCmd)
    {
         ...
         * ---BEGIN_APPLICATION_CODE---
         */
        rc = clAmfNodeSet(gClAmsMgmtHandle, pProvTxnData);
        if (rc != CL_OK)
        {
            clLogError("OI", "WRITE", 
                    "Unable to set node attribute, error %#x", rc);
        }
...
ClRcT clamfMgmtSAAMFNODETABLEProvRead(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnData)
{
    ...
    {
        case SAAMFNODETABLE_SAAMFNODEADMINSTATE:
            {
                ...
                if(rc != CL_OK)
                {
                    clLogError("OI", "READ", "Node [%s] config returned [%#x]",
                ...
                memcpy(pProvTxnData->pProvData, &pNodeConfig->adminState, pProvTxnData->size);
                clHeapFree(pNodeConfig);
                break;
            }
        case SAAMFNODETABLE_SAAMFNODEOPERATIONALSTATE:
            {
                ClAmsNodeStatusT *pNodeStatus = NULL;
                rc = clAmsMgmtEntityGetStatus(gClAmsMgmtHandle, &entity,
                        (ClAmsEntityStatusT**)&pNodeStatus);
                if(rc != CL_OK)
                {
                    clLogError("OI", "READ", "Node [%s] status returned [%#x]",
                            entity.name.value, rc);
                    return rc;
                }
                if(pProvTxnData->size != (ClUint32T)sizeof(pNodeStatus->operState))
                {
                    clLogError("OI", "READ", "Read size [%d] doesnt match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(pNodeStatus->operState));
                    clHeapFree(pNodeStatus);
                    return CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                }
                memcpy(pProvTxnData->pProvData, &pNodeStatus->operState, pProvTxnData->size);
                clHeapFree(pNodeStatus);
                break;
            }
        default:
 ...
 * Please refer protptype of this function in the file clamfMgmtSAAMFNODETABLEOAMPConfig.h
...
ClRcT clamfMgmtSAAMFNODETABLEProvObjectUpdate(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnDataList, ClUint32T txnDataEntries)
{
    ...
    for (i=0; i < txnDataEntries; i++)
    {
        ...
        switch (pProvTxnData->provCmd)
        {
             ...
             */
            {
                rc = clamfMgmtSAAMFNODETABLEProvUpdate(pThis, txnHandle, pProvTxnData);
                if(rc != CL_OK)
                    return rc;
            }
...
ClRcT clamfMgmtSAAMFNODETABLEProvObjectRead(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnDataList, ClUint32T txnDataEntries)
{
    ...
    for(i = 0; i < txnDataEntries; ++i)
    {
        rc = clamfMgmtSAAMFNODETABLEProvRead(pThis, txnHandle, pProvTxnDataList + i);
        if(rc != CL_OK)
            return rc;
    }
	/*
