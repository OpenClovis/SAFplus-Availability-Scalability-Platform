...
ClRcT clamfMgmtSAAMFSGTABLEProvConstructor( void *pThis, void *pUsrData, ClUint32T usrDataLen )
{
    ...
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectStart = clamfMgmtSAAMFSGTABLEProvObjectStart;      
    ((CL_OM_PROV_CLASS*)pThis)->clProvValidate = (fp)clamfMgmtSAAMFSGTABLEProvValidate;    
    ((CL_OM_PROV_CLASS*)pThis)->clProvUpdate = (fp)clamfMgmtSAAMFSGTABLEProvUpdate;    
    ((CL_OM_PROV_CLASS*)pThis)->clProvRollback = (fp)clamfMgmtSAAMFSGTABLEProvRollback;
    ((CL_OM_PROV_CLASS*)pThis)->clProvRead = (fp)clamfMgmtSAAMFSGTABLEProvRead;
	((CL_OM_PROV_CLASS*)pThis)->clProvObjectValidate = clamfMgmtSAAMFSGTABLEProvObjectValidate; 	 
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectUpdate = clamfMgmtSAAMFSGTABLEProvObjectUpdate; 	 
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectRollback = clamfMgmtSAAMFSGTABLEProvObjectRollback;
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectRead = clamfMgmtSAAMFSGTABLEProvObjectRead;
    ((CL_OM_PROV_CLASS*)pThis)->clProvObjectEnd = clamfMgmtSAAMFSGTABLEProvObjectEnd;      
 ...
 * clamfMgmtSAAMFSGTABLEOAMPConfig.h for detailed documentation for this function.
 */
...
ClRcT clamfMgmtSAAMFSGTABLEProvRead(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnData)
{
    ...
    ClAmsSGConfigT *pSGConfig = NULL;
    ClAmsSGConfigT sgConfig = {{0}};
    ClUint32T *pSUParam = NULL;
    ...
    rc = clAmsMgmtEntityGetConfig(gClAmsMgmtHandle, &entity,
            (ClAmsEntityConfigT**)&pSGConfig);
    if(rc != CL_OK)
    {
        clLogError("OI", "READ", "AMF entity get returned [%#x]", rc);
        return rc;
    }
    ...
    {
        case SAAMFSGTABLE_SAAMFSGADMINSTATE:
            {
                if((ClUint32T)sizeof(ClAmsAdminStateT) != pProvTxnData->size)
                {
                    clLogError("OI", "READ", "Read size [%d] does not match expected size [%d]",
                            pProvTxnData->size, (ClUint32T)sizeof(ClAmsAdminStateT));
                    return CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
                }
                memcpy(pProvTxnData->pProvData, &sgConfig.adminState, pProvTxnData->size);
            }
            break;

        case SAAMFSGTABLE_SAAMFSGNUMCURRASSIGNEDSU:
            {
                rc = clAmsMgmtGetSGAssignedSUList(gClAmsMgmtHandle, &entity,
                        &suList);
                if(rc != CL_OK)
                {
                    clLogError("OI", "READ", "SG assigned SU list returned [%#x]", rc);
                    return rc;
                }
                pSUParam = &suList.count;
                if(suList.entity) clHeapFree(suList.entity);
            }
            break;

        case SAAMFSGTABLE_SAAMFSGNUMCURRNONINSTANTIATEDSPARESU:
            {
                rc = clAmsMgmtGetSGInstantiableSUList(gClAmsMgmtHandle, &entity,
                        &suList);
                if(rc != CL_OK)
                {
                    clLogError("OI", "READ", "SG instantiable SU list returned [%#x]", rc);
                    return rc;
                }
                pSUParam = &suList.count;
                if(suList.entity) clHeapFree(suList.entity);
        ...
        case SAAMFSGTABLE_SAAMFSGNUMCURRINSTANTIATEDSPARESU:
            {
                rc = clAmsMgmtGetSGInServiceSpareSUList(gClAmsMgmtHandle, &entity,
                        &suList);
                if(rc != CL_OK)
                {
                    clLogError("OI", "READ", "SG inservice spare SU list returned [%#x]", rc);
                    return rc;
                }
                pSUParam = &suList.count;
                if(suList.entity) clHeapFree(suList.entity);
        ...
        default:
            clLogError("OI", "READ", "AMF mgr doesnt manage attribute [%d]", pProvTxnData->attrId);
    ...
    if(pSUParam)
    {
        if(pProvTxnData->size != (ClUint32T)sizeof(*pSUParam))
        {
            clLogError("OI", "READ", "Read size [%d] does not match expected size [%d]",
                    pProvTxnData->size, (ClUint32T)sizeof(*pSUParam));
            return CL_AMS_RC(CL_ERR_INVALID_PARAMETER);
        }
        memcpy(pProvTxnData->pProvData, pSUParam, pProvTxnData->size);
    }
 ...
 * Please refer protptype of this function in the file clamfMgmtSAAMFSGTABLEOAMPConfig.h
...
ClRcT clamfMgmtSAAMFSGTABLEProvObjectRead(CL_OM_PROV_CLASS* pThis, ClHandleT txnHandle, ClProvTxnDataT* pProvTxnDataList, ClUint32T txnDataEntries)
{
    ...
    for(i = 0; i < txnDataEntries; ++i)
    {
        rc = clamfMgmtSAAMFSGTABLEProvRead(pThis, txnHandle, pProvTxnDataList + i);
        if(rc != CL_OK)
            return rc;
    }
	/*
