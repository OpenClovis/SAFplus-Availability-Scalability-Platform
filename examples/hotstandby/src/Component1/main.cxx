...
//SAFplus::Mutex ckptMutex;
SAFplus::Checkpoint countCkpt;
...
//void timerCallback(Checkpoint *countCkpt)
static ClRcT timerCallback(void * countCkptArg)
{
    static uintcw_t count = 1;
    char vdata[sizeof(Buffer)-1+sizeof(uintcw_t)];
    Buffer *val = new(vdata) Buffer(sizeof(uintcw_t));
    *((uintcw_t*)val->data) = count*2;
    clprintf(SAFplus::LOG_SEV_INFO,"update key = %x,value = %x",count,*((uintcw_t*)val->data));
    countCkpt.write(count,*val);
    count++;
    sleep(5);
    return 0;
}

static ClRcT hotStandbyCallback(const Buffer& key, const Buffer& value, void *cookie)
{
	clprintf(SAFplus::LOG_SEV_INFO,"hotStandbyCallback received update key[%x] value[%x]",*((uintcw_t*)key.data), *((uintcw_t*)value.data));
}
                   ...
                   SaAmfCSIDescriptorT csiDescriptor)
{
    ...
    switch ( haState )
    {
        ...
        case SA_AMF_HA_ACTIVE:
        {
            ...
            pthread_t thr;
            clprintf(SAFplus::LOG_SEV_INFO,"HotStandby app: ACTIVE state requested; activating service");
            running = 1;
            //pthread_create(&thr,NULL,activeLoop,NULL);
            ClRcT rc;
            if((rc=timerInitialize(NULL)) != CL_OK)
            {
                clprintf(SAFplus::LOG_SEV_INFO,"Failed to initialize timer");
            }
            static TimerTimeOutT  TimeOut;
            TimeOut.tsMilliSec=0;
            TimeOut.tsSec=1;
            
            clprintf(SAFplus::LOG_SEV_INFO,"HotStandby app: ACTIVE state requested; call ");
            rc =myClock.timerCreate(TimeOut,TimerTypeT::TIMER_REPETITIVE,TimerContextT::TIMER_TASK_CONTEXT,timerCallback,NULL);
            rc = myClock.timerStart();
        ...
        case SA_AMF_HA_STANDBY:
        {
            ...
            clprintf(SAFplus::LOG_SEV_INFO,"HotStandby app: Standby state requested");
            running = 2;
            countCkpt.clCkptImmediateConsumptionRegister(hotStandbyCallback,NULL);
            /* The AMF times the interval between the assignment and acceptance
               of the work (the time interval is configurable).
               So it is important to call this saAmfResponse function ASAP.
             */
        ...
        case SA_AMF_HA_QUIESCED:
        {
             ...
             */
            clprintf(SAFplus::LOG_SEV_INFO, "HotStandby app: Acknowledging new state quiesced");
            running = 0;
        ...
        case SA_AMF_HA_QUIESCING:
        {
            ...
            if (1)
              ...
              clprintf(SAFplus::LOG_SEV_INFO, "HotStandby app: Signaling completion of QUIESCING");
              running = 0;
  ...
  do
    {
      ...
      if (running==1) clprintf(SAFplus::LOG_SEV_INFO,"HotStandby app: Active.  Hello World!"); // show_progress());
      else if (running==2) clprintf(SAFplus::LOG_SEV_INFO,"HotStandby app: Standby."); // show_progress());
      else 
        {
        clprintf(SAFplus::LOG_SEV_INFO,"HotStandby app: idle");
