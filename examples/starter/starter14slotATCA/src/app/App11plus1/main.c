...
ClRcT GetVaiFromCsi(const SaNameT       *compName,const char* name, VirtualIpAddress* ret)
{
    ...
    else
    {
        ...
        for(i = 0; i < csiRef.numCSIs; ++i)
        {
            ...
            if (strncmp((const char *) pCSI->csiDescriptor.csiName.value,name, pCSI->csiDescriptor.csiName.length) ==0)
            {                        
...
void HandleCsiChange(char* op,const SaNameT* compName, ClAmsCSIDescriptorT* csiDescriptor)
{
            ...
            {
                VirtualIpAddress temp;
                if (CL_OK == GetVaiFromCsi(compName,(const char *) csiDescriptor->csiName.value, &temp)) AddRemVirtualAddress(op,&temp);
            }        
