...
void safTerminate(SaInvocationT invocation, const SaNameT *compName)
{
    ...
    clprintf (CL_LOG_SEV_INFO, "Component [%.*s] : PID [%d]. Terminated", compName->length, compName->value, mypid);
...
#define min(x,y) ((x < y)? x: y)
    ...
    if (rc != SA_AIS_OK)
    {
        logrc(CL_LOG_SEV_ERROR, "%s: Failed to init event mechanism [0x%x]\n", appname, rc);
        errorExit(rc);
    }
    ...
    if (rc != SA_AIS_OK)
    {
        clprintf(CL_LOG_SEV_ERROR, "%s\t:Failure opening event channel[0x%x] \n at %ld", appname, rc, time(0L));
        logrc(CL_LOG_SEV_ERROR, "%s\t:Failure opening event channel[0x%x]\n ", appname, rc);
        errorExit(rc);
    }
    ...
    if (rc != SA_AIS_OK)
    {
        ...
        logrc(CL_LOG_SEV_ERROR,"%s\t:Failed to subscribe to event channel [0x%x]\n ", appname, rc);
        errorExit(rc);
    }
...
static void  appEventCallback( SaEvtSubscriptionIdT	subscriptionId, SaEvtEventHandleT     eventHandle,  SaSizeT eventDataSize)
{
    ...
    if (resTest == 0)
    {
        logmsg(CL_LOG_SEV_ERROR, "%s\t:Failed to allocate space for event", appname);
        return;
    }
    *(((char *)resTest) + eventDataSize) = 0;
    ...
    if (saRc!= SA_AIS_OK)
    {
        logrc(CL_LOG_SEV_ERROR, "%s\t:Failed to get event data [0x%x]",appname, saRc);
    }
