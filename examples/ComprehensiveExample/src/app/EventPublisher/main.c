...
{
     0,
     0,                 // CPM service instance handle
     0,
     0,
     0,
     // callbacks for CPM interaction
     {
         NULL,
         safTerminate,
         safAssignWork,
    ...
    // Expected/Required for CPM service API
    {
        'B',
         01,
         01
    ...
    {
      sizeof (PUBLISHER_NAME) - 1,
      PUBLISHER_NAME
    },
    1,              // 0 => suspended; 1 => resumed
    0,              // flag that tells main loop to exit
    SA_AMF_HA_STANDBY
};

static inline void logrc(ClLogSeverityT severity, char *buffer, char *appname, ClRcT rc)
{
    clLogWrite(CL_LOG_HANDLE_APP, severity, NULL, buffer, appname, rc);
}

static inline void logmsg(ClLogSeverityT severity, char *buffer, char *appname)
{
    clLogWrite(CL_LOG_HANDLE_APP, severity, NULL, buffer, appname);
}

static SaAisErrorT appPublishEvent();
                   ...
                   SaAmfCSIDescriptorT csiDescriptor)
{
    ...
    switch ( haState )
    {
        ...
        case SA_AMF_HA_ACTIVE:
        {
             ...
             */
            clprintf(CL_LOG_SEV_INFO,"ACTIVE state request: activating service");
            gTestInfo.running = 1;
            saAmfResponse(amfHandle, invocation, SA_AIS_OK);
            break;
        }
        ...
        case SA_AMF_HA_STANDBY:
        {
            ...
            /* The AMF times the interval between the assignment and acceptance
               ...
               So it is important to call this saAmfResponse function ASAP.
             */
            clprintf(CL_LOG_SEV_INFO,"New state is not ACTIVE: deactivating service");
            gTestInfo.running = 1;
            saAmfResponse(amfHandle, invocation, SA_AIS_OK);  
            break;
        }
        ...
        case SA_AMF_HA_QUIESCED:
        {
             ...
             */
            gTestInfo.running = 0;
            clprintf(CL_LOG_SEV_INFO,"%s: Acknowledging new state", appname);
            saAmfResponse(amfHandle, invocation, SA_AIS_OK);
            break;
        }
...
void initializeAmf(void)
{
    ...
    rc = saEvtInitialize(&gTestInfo.evtInitHandle, &evtCallbacks, &gTestInfo.evtVersion);
    if (rc != SA_AIS_OK)
    {
    ...
    rc = saEvtChannelOpen (gTestInfo.evtInitHandle, &gTestInfo.evtChannelName, (SA_EVT_CHANNEL_PUBLISHER | SA_EVT_CHANNEL_CREATE),
                             (ClTimeT)SA_TIME_END, &gTestInfo.evtChannelHandle);
    if (rc != SA_AIS_OK)
    {
    ...
    rc = saEvtEventAllocate(gTestInfo.evtChannelHandle, &gTestInfo.eventHandle);
    if (rc != SA_AIS_OK)
    {
    ...
    rc = saEvtEventAttributesSet(gTestInfo.eventHandle, NULL, 1, 0, &gTestInfo.publisherName);
    if (rc != SA_AIS_OK)
    {
    ...
    clOsalTaskCreateDetached("testEvtMainLoop", CL_OSAL_SCHED_OTHER, CL_OSAL_THREAD_PRI_NOT_APPLICABLE, 65536,testEvtMainLoop,(void*)appname);
...
static void generate_time_of_day(char **data, ClSizeT *data_len)
{
    time_t t;
    ...
    {
        logmsg(CL_LOG_SEV_ERROR,"%s\t: generate_time_of_day passed null pointer", appname);
        return;
    }
...
static void generate_load_average(char **data, ClSizeT *data_len)
{
    ...
    if (data == 0 || data_len == 0)
    {
        logmsg(CL_LOG_SEV_ERROR, "%s\t: generate_load_average passed null pointer ", appname);
        return;
    }
    ...
    {
        logmsg(CL_LOG_SEV_ERROR, "%s\t: failed to open /proc/loadavg", appname);
        return;
    }
    ...
    *data = clHeapAllocate(*data_len);
    if (data == 0)
    {
        logmsg(CL_LOG_SEV_ERROR, "%s: failed to allocate memory for loadavg contents\n", appname);
        *data_len = 0;
        close(fd);
        return;
    }
...
static void* testEvtMainLoop(void *thdParam)
{
    ...
    logmsg(CL_LOG_SEV_DEBUG, "%s exited main loop", appname);
