 ...
 *
 ******************************************************************************
 * This code is auto-generated by Clovis Works @Version 2.0
 *
 ***************************** Description ************************************
 *
 * 
...
def processComponentList(compList, resMap, alarmMap):
	...
	for comp in compList:
				...
				if len(alarm) > 0 and alarm[0].attributes["isEnabled"].value=="true":
					...
					if len(alarmids) > 0:
						for alarmid in alarmids:
							...
							d = dict();
							d["macroID"] = macroID * 100 
							macroName = alarmProfile.attributes["ProbableCause"].value
							d["macroName"] = macroName.upper()
							macroID = macroID + 1
							tempMacroName = tempAlarmMap.get(macroName)
							if tempMacroName == None:
								macros += macroTemplate.safe_substitute(d)
							...
							if len(genRule) > 0 and len(genRule[0].getElementsByTagName("AlarmIDs")) > 0:
								if tempMacroName == None:
									alarmRulesTables += createGenRuleTable(genRule[0], alarmProfileName, alarmMap)
							...
							if len(suppRule) > 0 and len(suppRule[0].getElementsByTagName("AlarmIDs")) > 0:
								if tempMacroName == None:
									alarmRulesTables += createSuppRuleTable(suppRule[0], alarmProfileName, alarmMap)
							
							alarmProfiles += createAlarmProfileDef(alarmProfile, genRule, suppRule)
							if tempMacroName == None:
								tempAlarmMap[macroName] = macroName
		...
		result += createComponentAlarmTable(compAlarms, alarmProfilesTables, alarmRulesTables, macros, compName)
...
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
def createComponentAlarmTable(compAlarms, alarmProfilesTables, alarmRules, macros, name):
	map = dict();
...
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
def createAlarmProfileDef(alarmProfile, genRule, suppRule):
	alarmMap = dict();
	...
	if len(genRule) > 0 and len(genRule[0].getElementsByTagName("AlarmIDs")) > 0:
		alarmMap["genRuleTableName"] = "Cl" + alarmProfile.attributes["ProbableCause"].value + "GenRule"
	else:
		alarmMap["genRuleTableName"] = "NULL"
	if len(suppRule) > 0 and len(suppRule[0].getElementsByTagName("AlarmIDs")) > 0:
		alarmMap["suppressRuleTableName"] = "Cl" + alarmProfile.attributes["ProbableCause"].value +"SuppressRule" 
	else:
