...
{\
	...
	'BITS': ('UINT32', 1),\
	'TruthValue': ('UINT8', 1),\
	'OCTET STRING': ('UINT8',256),\
	'IpAddress': ('UINT8',4),\
	'NetworkAddress': ('UINT8',4),\
	'OPAQUE': ('UINT8',256),\
	'UTF8String': ('UINT8',256),\
	'IA5String': ('UINT8',256),\
	'DisplayString': ('UINT8', 256),\
}
...
def processAlarmdataList(alarmdata):
	...
	for alarm in alarmdata:	
		...
		cwkey = alarm.attributes["CWKEY"].value
				...
				if node.nodeName == "Provisioning":
					...
					if len(pronodes) > 0:
						for pronode in pronodes:
							if pronode.nodeName == "Attributes":
...
#alarmdataXmi = sys.argv[1]+'/'+sys.argv[3]+'/models/alarmdata.xmi'
#resourcedataXmi = sys.argv[1]+'/'+sys.argv[3]+'/models/resourcedata.xmi'
alarmdataXmi = sys.argv[1]+'/models/alarmdata.xmi'
resourcedataXmi = sys.argv[1] +'/models/resourcedata.xmi'
...
if os.path.exists(alarmdataXmi) and os.path.exists(resourcedataXmi):	
	...
	resourcedataList = resourcedataDoc.getElementsByTagName("resource:ChassisResource") + resourcedataDoc.getElementsByTagName("resource:HardwareResource") + resourcedataDoc.getElementsByTagName("resource:SoftwareResource") + resourcedataDoc.getElementsByTagName("resource:SystemController") + resourcedataDoc.getElementsByTagName("resource:NodeHardwareResource")
