...
def generate_db_for_node():
    bin_dir = get_asp_bin_dir()
    model = [name for name in os.listdir(bin_dir) if name.endswith("Model.xml")]
    if len(model) != 1:
        log.info("There is more/less than one *Model.xml. Recheck and try again...")
        sys.exit(1)
    cmd = "%s/safplus_db -x %s safplusAmf" % (bin_dir,model[0])
    log.info(cmd)
    os.system(cmd)
