...
ClIdlHandleT clockExternalServerHandle = 0;
...
#define __RPC_SERVER_ADDRESS __LOGICAL_ADDRESS(1)
#define __RPC_SERVER_EXTERNAL_ADDRESS __LOGICAL_ADDRESS(4)
void RmdConnect(void);
void RmdExternalConnect(void);
void *SendRmdMsg(void *unused)
{
	clprintf (CL_LOG_SEV_DEBUG, "Enter SendRmdMsg thread");
	ClRcT clrc;
	while(1)
	{
		acTimeT_4_0_0 time;
		if (clockServerHandle == 0)
			RmdConnect();  // initialize the client
		if ((clrc=GetTimeClientSync_4_0_0(clockServerHandle, &time))==CL_OK)
		{
		     clprintf(CL_LOG_SEV_INFO,"Get Time RPC [ from SCComponent ]: [%02d:%02d:%02d]",
		        	    time.hour,time.minute,time.second);
		}
        else
        {
            clprintf(CL_LOG_SEV_ERROR,"Get Time RPC [ from SCComponent ] failed %x", clrc);
        }
		sleep(5);
	}
    return NULL;
...
void *SendExternalRmdMsg(void *unused)
{
	ClRcT clrc;
	while(1)
	{
		acTimeT_4_0_0 time;
		if (clockExternalServerHandle == 0)
			RmdExternalConnect();  // initialize the client
		if ((clrc=GetTimeClientSync_4_0_0(clockExternalServerHandle, &time))==CL_OK)
		{
		     clprintf(CL_LOG_SEV_INFO,"Get Time RPC sync[ From External App ]: [%02d:%02d:%02d]",
		        	    time.hour,time.minute,time.second);
		}
        else
        {
            clprintf(CL_LOG_SEV_ERROR,"Get Time RPC sync[ From External App ] failed %x", clrc);
        }
		sleep(5);
		if ((clrc=GetTimeClientAsync_4_0_0(clockExternalServerHandle,&time,(Alarm_clock_EOGetTimeAsyncCallbackT_4_0_0)printTime,NULL))==CL_OK)
		{
		     //clprintf(CL_LOG_SEV_INFO,"Get Time RPC Async External successful",clrc);
		}
        else
        {
            clprintf(CL_LOG_SEV_ERROR,"Get Time RPC async[ From External App ] failed %x", clrc);
        }
		sleep(5);
	}
    return NULL;
                   ...
                   SaAmfCSIDescriptorT csiDescriptor)
{
    ...
    switch ( haState )
    {
        ...
        case SA_AMF_HA_ACTIVE:
        {
             ...
             */
        	pthread_t send_thr,send_external_thr;
       	    pthread_create(&send_thr,NULL,SendRmdMsg,NULL);
       	    pthread_create(&send_external_thr,NULL,SendExternalRmdMsg,NULL);
            saAmfResponse(amfHandle, invocation, SA_AIS_OK);
            break;
        }
...
void RmdConnect(void)
{
    ...
    serverCompAddr.address.iocAddress.iocLogicalAddress = __RPC_SERVER_ADDRESS;
    /*
    ...
    if(rc != CL_OK)
    {
        clprintf(CL_LOG_SEV_ERROR,"Cannot initialize the RMD server handle");
    }
...
void RmdExternalConnect(void)
{
    ...
    serverCompAddr.addressType = CL_IDL_ADDRESSTYPE_IOC;
    serverCompAddr.address.iocAddress.iocLogicalAddress = __RPC_SERVER_EXTERNAL_ADDRESS;
    /*
     * idl related information.
     */
    ...
    if(!clockServerHandle)
    {
        rc = clIdlHandleInitialize(&idlObj, &clockExternalServerHandle);
    }
