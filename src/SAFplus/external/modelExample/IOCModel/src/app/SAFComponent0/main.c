...
ClIocCommPortHandleT iocCommPortSend;
ClIocCommPortHandleT iocCommPortRev;
ClUint8T mSendProtoId;
ClUint8T mReceiveProtoId;
...
void *SendIocMsg(void *unused)
{
	clprintf (CL_LOG_SEV_DEBUG, "Enter SendIocMsg thread");
	while(1)
	{
		sendIocLoop(iocCommPortSend,mSendProtoId,'S');
		sleep(10);
	}
    return NULL;
}

void *RevIocMsg(void *unused)
{
	clprintf (CL_LOG_SEV_DEBUG, "Enter RevIocMsg thread");
	recvIocLoop(iocCommPortRev);
    return NULL;
}
...
void safTerminate(SaInvocationT invocation, const SaNameT *compName)
{
    ...
    /*
     * Unregister with AMF and respond to AMF sayingioc_port_dest whether the
                   ...
                   SaAmfCSIDescriptorT csiDescriptor)
{
    ...
    switch ( haState )
    {
        ...
        case SA_AMF_HA_ACTIVE:
        {
             ...
             */
        	clprintf (CL_LOG_SEV_INFO,"create a commport ");
        	ClRcT retCode = clIocCommPortCreate(0, CL_IOC_UNRELIABLE_MESSAGING, &iocCommPortSend);
       	    if(retCode != CL_OK)
       	    {
       	    	clprintf(CL_LOG_SEV_INFO,"Error : Failed to create a commport. rc 0x%x\n", retCode);
       	        exit(0);
       	    }
       	    ClIocPortT port = TEST_IOC_PORT;
        	retCode = clIocCommPortCreate(port, CL_IOC_UNRELIABLE_MESSAGING, &iocCommPortRev);
       	    if(retCode != CL_OK)
       	    {
       	    	clprintf(CL_LOG_SEV_INFO,"Error : Failed to create a commport rev . rc 0x%x\n", retCode);
       	        exit(0);
       	    }
       	    clprintf (CL_LOG_SEV_INFO,"Successful to create a commport : 0x%x\n ",port);
        	mSendProtoId = CL_IOC_USER_PROTO_START + 2;
        	mReceiveProtoId= CL_IOC_USER_PROTO_START +1;
       	    pthread_t read_thr,write_thr;
       	    pthread_create(&read_thr,NULL,RevIocMsg,NULL);
       	    pthread_create(&write_thr,NULL,SendIocMsg,NULL);
            saAmfResponse(amfHandle, invocation, SA_AIS_OK);
            break;
        }
