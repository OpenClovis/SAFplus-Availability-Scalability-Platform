...
void *SendRmdMsg(void *unused)
{
	...
	while(1)
	{
		...
		{
		     clprintf(CL_LOG_SEV_INFO,"Get Time RPC [ INTERNAL ] returned :    [%02d:%02d:%02d]",
		        	    time.hour,time.minute,time.second);
		}
        else
        {
            clprintf(CL_LOG_SEV_ERROR,"Get Time RPC failed %x", clrc);
        }
		sleep(5);
	}
    return NULL;
...
void *SendExternalRmdMsg(void *unused)
{
	...
	while(1)
	{
		...
		{
		     clprintf(CL_LOG_SEV_INFO,"Get Time RPC [ EXTERNAL ] returned :    [%02d:%02d:%02d]",
		        	    time.hour,time.minute,time.second);
		}
        else
        {
            clprintf(CL_LOG_SEV_ERROR,"Get Time RPC [ EXTERNAL ] failed %x", clrc);
        }
		sleep(5);
        ...
        {
            clprintf(CL_LOG_SEV_ERROR,"Get Time RPC ASYNC External failed %x", clrc);
        }
		sleep(5);
	}
    return NULL;
