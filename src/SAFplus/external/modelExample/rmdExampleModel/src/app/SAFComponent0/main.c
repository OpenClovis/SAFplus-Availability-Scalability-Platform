...
ClRcT RmdInitialize(ClIocLogicalAddressT addr, ClIocPortT port);
...
void initializeAmf(void)
{
    ...
    RmdInitialize(__RPC_SERVER_ADDRESS, iocPort);
}
...
ClRcT RmdInitialize(ClIocLogicalAddressT addr, ClIocPortT port)
{
    ...
    clprintf(CL_LOG_SEV_INFO, "Enter RmdInitialize");
    if ( (clrc = clAlarm_clock_EOClientInstall()) != CL_OK)  // Actually ignore the name, it starts the server
    {
        clprintf (CL_LOG_SEV_ERROR, "RPC server initialization failed. error: 0x%x", clrc);
    }

    if ( (clrc = clAlarm_clock_EOClientTableRegister(port) ) != CL_OK)
    {
        clprintf (CL_LOG_SEV_ERROR, "RPC server client initialization failed. error: 0x%x", clrc);
    }

    tlInfo.physicalAddr.nodeAddress = clIocLocalAddressGet();
    clprintf (CL_LOG_SEV_ERROR, "RPC server nodeAddress: %d", tlInfo.physicalAddr.nodeAddress);
    tlInfo.physicalAddr.portId = port;
    tlInfo.haState = CL_IOC_TL_ACTIVE;
    tlInfo.logicalAddr = addr;
    clprintf (CL_LOG_SEV_ERROR, "RPC server logicalAddr: %d", tlInfo.physicalAddr.nodeAddress);
    clprintf (CL_LOG_SEV_ERROR, "RPC server iocport: %d", port);
    if ( (clrc = clIocTransparencyRegister(&tlInfo) ) != CL_OK )
    {
        clprintf(CL_LOG_SEV_ERROR, "RPC server transparency layer init failed with [%#x]", clrc);
    }
    clprintf(CL_LOG_SEV_INFO, "finish Initialize");
    return clrc;
}
