...
ClHandleT handle=-1;
extern ClHpiEventFilterT test_filter[];
...
ClRcT clTestHpiEventFilter(void)
{
    ClRcT rc;
    ClHpiEventFilterTableT filterTable;


    filterTable.noOfFilters = getNumberOfFilters();
    filterTable.filter = test_filter;

    rc = clHpiEventFilterRegister(&filterTable, &handle);
    if(rc != CL_OK)
    {
        CL_DEBUG_PRINT(CL_DEBUG_ERROR,("Cannot Register for Hpi Events:%x\n",rc));
        return rc;
    }
    return CL_OK;
}
    ...
    ClCharT *argv[])
{
    ...
    rc = clTestHpiEventFilter();
    if(rc != CL_OK)
        CL_DEBUG_PRINT(CL_DEBUG_ERROR,("Cannot test the Hpi Event Filter capability\n"));
...
{
    clHpiEventFilterUnregister(handle);
    return CL_OK;
}
