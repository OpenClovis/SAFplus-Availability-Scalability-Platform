...
**   Availability(TM) Forum HPI-to-AdvancedTCA(TM) Mapping Specification.
**   It contains all of the prototypes and type definitions. Note, this file
**   was generated from the HPI-to-AdvancedTCA(TM) Mapping Specification 
**   document.
**   
...
** Color Capabilities
**
** This definition defines the possible color capabilities for an individual 
...
typedef enum {
    ATCAHPI_LED_AUTO,       /*Set all LEDs under this Resource to Auto mode */
    ATCAHPI_LED_MANUAL,     /*Set all LEDs under this Resource to Manual mode*/
    ATCAHPI_LED_LAMP_TEST   /*Put all LEDs under this Resource in Lamp Test */
...
** that this is a Control that aggregates all LEDs of a particular Resource.
*/
...
** ATCAHPI_ENT_SHELF_FRU_DEVICE_SLOT   - ATCA Shelf FRU information device slot
...
**
** ATCAHPI_ENT_PICMG_FRONT_BLADE       - ATCA Field Replaceable Front Board
** ATCAHPI_ENT_SHELF_FRU_DEVICE        - ATCA Shelf FRU Information Device
...
** Note that the Control numbers from ATCAHPI_CTRL_NUM_FRU_POWER_ON_SEQUENCE to 
** ATCAHPI_CTRL_NUM_FRU_POWER_ON_SEQUENCE + FEh are reserved for FRU Power On 
** Sequence Controls.
*/
...
typedef SaHpiUint8T AtcaHpiCsCurrentPwrState;    /* Maps to OEM Control state  
                                                    byte [2] for the Chassis 
                                                    Status Control */
...
#define ATCAHPI_CS_CPS_PWR_CTRL_FAULT            (AtcaHpiCsCurrentPwrState)0x10 
...
typedef SaHpiUint8T AtcaHpiCsLastPwrEvent;       /* maps to OEM Control state 
                                                    byte [3]for the Chassis 
                                                    Status Control */
...
/* If bit is set, last power down caused by a power interlock being activated */
...
**   ATCAHPI_SENSOR_NUM_IPMB0 + 0x5F are reserved for systems supporting a 
...
** Note that the Control numbers from ATCAHPI_CTRL_NUM_AMC_POWER_ON_SEQUENCE to 
** ATCAHPI_CTRL_NUM_AMC_POWER_ON_SEQUENCE + Fh are reserved for AMC Power On 
** Sequence Controls.
*/
...
/*
** Well-known OEM values assigned to RDR entries for OEM Controls defined by 
** this document.
*/
