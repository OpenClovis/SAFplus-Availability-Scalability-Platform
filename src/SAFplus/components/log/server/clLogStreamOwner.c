...
clLogStreamOwnerGlobalBootup(void) 
{
    ...
    if( !(pCommonEoEntry->flags & __LOG_EO_INIT)
        ||
        (pCommonEoEntry->flags & __LOG_EO_SO_INIT))
    {
...
clLogStreamOwnerGlobalShutdown(void)
{
    ...
    rc = clLogStreamOwnerEoEntryGet(&pSoEoEntry, &pSvrCommonEoData);
    if( CL_OK != rc )
    {
        return rc;
    }
    clOsalMutexLock(&pSvrCommonEoData->lock);
    if(!(pSvrCommonEoData->flags & __LOG_EO_INIT)
       ||
       !(pSvrCommonEoData->flags & __LOG_EO_SO_INIT))
    {
        clOsalMutexUnlock(&pSvrCommonEoData->lock);
        return rc;
    }
    pSvrCommonEoData->flags &= ~__LOG_EO_SO_INIT;
    clOsalMutexUnlock(&pSvrCommonEoData->lock);
