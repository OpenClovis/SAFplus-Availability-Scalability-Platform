...
ClRcT _ckptCheckpointReadSectionsClientSync_6_0_0(CL_IN ClIdlHandleT handle, CL_IN ClHandleT ckptHdl, CL_OUT ClCkptIOVectorElementT_4_0_0** pIOVec, CL_OUT ClUint32T* pNumVecs)
{
    ClRcT rc = CL_OK;
    ClVersionT funcVer = {6, 0, 0};
    ClUint32T funcNo = CL_EO_GET_FULL_FN_NUM(CL_EO_NATIVE_COMPONENT_TABLE_ID, 52);
    ClBufferHandleT inMsgHdl = 0;
    ClBufferHandleT outMsgHdl = 0;
    ClIocAddressT address = {{0}};
    ClIdlHandleObjT* pHandleObj = NULL;
    ...
    rc = clXdrUnmarshallClUint32T( outMsgHdl, pNumVecs);
    if (CL_OK != rc)
    {
        return rc;
    }

    rc = clXdrUnmarshallPtrClCkptIOVectorElementT_4_0_0( outMsgHdl, (void **)pIOVec, *pNumVecs);
    if (CL_OK != rc)
    {
        return rc;
    }
