...
namespace SAFplus
{
    ...
    void App::init(char releaseCode, int majorVersion, int minorVersion)
    {
        ...
        callbacks.saAmfComponentTerminateCallback = NULL; /* Done it in destructor */
        ...
        if ((rc = saAmfSelectionObjectGet(amfHandle, &dispatch_fd)) != SA_AIS_OK)
            throw new clAppException("Initialization error", rc);
        ...
        if ((rc = saAmfComponentNameGet(amfHandle, &appName)) != SA_AIS_OK)
            throw new clAppException("Getting AMF handle name error", rc);
        if ((rc = saAmfComponentRegister(amfHandle, &appName, NULL)) != SA_AIS_OK)
            throw new clAppException("Register application to AMF error", rc);
    ...
    void AppThreadForWork::ActivateWorkAssignment(const ClAmsCSIDescriptorT& workDescriptor)
    {
        ...
        clDbgCheck(ws->state != CL_AMS_HA_STATE_ACTIVE, return,
                        ("Double activation of work assignment %s, ignoring", workDescriptor.csiName.value ));
    ...
    void AppThreadForWork::StandbyWorkAssignment(const ClAmsCSIDescriptorT& workDescriptor)
    {
        ...
        clDbgCheck(ws->state != CL_AMS_HA_STATE_STANDBY, return,
                        ("Double activation of work assignment %s, ignoring", workDescriptor.csiName.value ));
    ...
    void AppThreadForWork::QuiesceWorkAssignment(const ClAmsCSIDescriptorT& workDescriptor)
    {
        ...
        clDbgCheck(ws->state != CL_AMS_HA_STATE_QUIESCING, return,
                        ("Double quiesce of work assignment %s, ignoring", workDescriptor.csiName.value ));
