...
namespace SAFplus
  {
  MgtVectorList::MgtVectorList(const char* name) :
    MgtObject(name)
    {
    }

  MgtVectorList::~MgtVectorList()
    {
    }

#if 0
  ClRcT MgtVectorList::addKey(std::string key)
    {
    ClRcT rc = CL_OK;
  ...
  ClRcT MgtVectorList::removeKey(std::string key)
    {
  ...
  void MgtVectorList::removeAllKeys()
    {
    mKeys.clear();
    }
#endif
  ...
  ClBoolT MgtVectorList::isEntryExist(MgtObject* entry)
    {
...
#if 0
  MgtObject* MgtVectorList::findEntryByKeys(std::map<std::string, std::string> *keys)
    {
    ClUint32T i, j;
  ...
  ClRcT MgtVectorList::removeEntry(ClUint32T index)
    {
    ClRcT rc = CL_ERR_NOT_EXIST;
  ...
  void MgtVectorList::removeAllChildren()
    {
    mEntries.clear();
    }

  MgtObject* MgtVectorList::getEntry(ClUint32T index)
    {
    if ((0 <= index) && (index < mEntries.size()))
      {
  ...
  ClUint32T MgtVectorList::getEntrySize()
    {
    return (ClUint32T) mEntries.size();
    }

  void MgtVectorList::toString(std::stringstream& xmlString)
    {
    ClUint32T i;
  ...
  ClBoolT MgtVectorList::set(void *pBuffer, ClUint64T buffLen, SAFplus::Transaction& t)
    {
  ...
  ClRcT MgtVectorList::removeChildObject(const std::string& objectName)
    {
  ...
  ClRcT MgtVectorList::addChildObject(MgtObject *mgtObject, const std::string& objectName)
    {
    ClRcT rc = CL_OK;
    assert(mgtObject);
