...
#include "unitTest/ServiceGroup.hxx"
...
void testLoadMultikey()
{
    ...
    // Manual testing
    // 1. Auto key generated
    UnitTestRoot testAutoGeneratedKey;
    unitTest::ServiceGroup sg0;
    sg0.name.value="sg0";
    sg0.id=1;
    sg0.data=11;

    unitTest::ServiceGroup sg1;
    sg1.name.value="sg1";
    sg1.id=2;
    sg1.data=22;

    testAutoGeneratedKey.serviceGroupList.addChildObject(&sg0);
    testAutoGeneratedKey.serviceGroupList.addChildObject(&sg1);
    stringstream ss;
    testAutoGeneratedKey.toString(ss);
    logDebug("MGT", "TEST1", "%s", ss.str().c_str());

    // 2. Auto key generated that already exists
    UnitTestRoot testAutoGeneratedKeyIsAlreadyExist;
    sg1.name.value="sg0";
    try
    {
      testAutoGeneratedKeyIsAlreadyExist.serviceGroupList.addChildObject(&sg0);
      testAutoGeneratedKeyIsAlreadyExist.serviceGroupList.addChildObject(&sg1);
    }
    catch (MgtError &e)
    {
      logDebug("MGT", "TEST2", "%s", e.what());
    }

    // 3. Key provide is already existing
    UnitTestRoot testKeyIsAlreadyExist;
    try
    {
      testKeyIsAlreadyExist.serviceGroupList.addChildObject(&sg0, "sg1");
      testKeyIsAlreadyExist.serviceGroupList.addChildObject(&sg1, "sg1");
    }
    catch (MgtError &e)
    {
      logDebug("MGT", "TEST3", "%s", e.what());
    }

    // 4. Test object->get
    string ssRoot;
    root.get(&ssRoot);
    logDebug("MGT", "TEST4", "%s", ssRoot.c_str());
