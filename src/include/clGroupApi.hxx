...
/*? <desc order="1"><html><p>
Group membership is a very powerful feature that lets services in the cluster discover and organize.  Group membership not limited on a per node or per application basis.  You may create multiple Group objects within an application or library and have that Group object join and query groups.  For example, let us propose a storage service that stores data to a distributed database.  This service runs as a "server" application on many nodes within the cluster and can use this group membership facility to discover all the other storage servers on the cluster, and (if desired) to pick two specific server instances to assume an "active" and "standby" role.  Note that the Group membership subsystem simply consistently designates 2 "special" server instances across every node in the cluster -- the storage service can the apply whatever semantics (for example active/standby) it wants to these special instances -- or apply no semantics whatsoever if the servers are fully peer-to-peer.
</p>
<p>
The storage service can provide a client library so applications can access the service.  This client library can use a Group object to discover all of the storage servers, without actually joining the group itself.  So the group membership feature can be used both to discover an application's peers, and to discover services that the application requires.
</p>
</html>
</desc>
*/
