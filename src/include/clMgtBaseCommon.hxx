...
static std::string json2xmlnoheader(std::stringstream& strJson)
{
  ...
  std::stringstream ss;
  write_xml_element(ss,ptree::key_type(),pt,-1,xml_writer_settings<ptree::key_type::value_type>());
  return ss.str();
}
...
std::string getXMLChild(const std::string& tagName)
{
	std::string retStr= "";
	try{
			boost::property_tree::ptree noderoot = pt.get_child(tagName);
			std::stringstream ss;
			//write_xml(ss, noderoot);
...
std::map<std::string, std::string> getChildNameValues(const std::string& tagName)
{
	...
	try{
	boost::property_tree::ptree noderoot = pt.get_child(tagName);
	BOOST_FOREACH(boost::property_tree::ptree::value_type &vs,noderoot)
	{
...
void getLevel(const boost::property_tree::ptree& ppt,const int& inLevel, int& outLevel)
{
