...
#define Log(severity,...) clAppLog(CL_LOG_HANDLE_APP, severity, 10, "DYN", "SVR", __VA_ARGS__)
...
static void checkError(const char* operation, ClRcT rc)
{
  if (rc != 0)
    {
      Log(CL_LOG_SEV_ERROR,"%s: Error 0x%x", operation, rc);
...
static void saNameAppend(SaNameT* name, const char* suffix)
{
  ...
  strncpy((char *)&name->value[name->length],suffix,CL_MAX_NAME_LENGTH-name->length);
  name->length += amt2cpy;
...
void removeNode(const char* safName)
{
  ...
  if ((rc != CL_OK)&&(CL_GET_ERROR_CODE(rc) != CL_ERR_DOESNT_EXIST)) checkError("Get node address",rc);
  if (CL_GET_ERROR_CODE(rc) != CL_ERR_DOESNT_EXIST)
    {
      if ((rc = clCpmNodeShutDown(slotInfo.nodeIocAddress))  != CL_OK) checkError("Shutdown SAFplus on node",rc);
      ClAmsNodeStatusT* ns;
      bool done = false;
      do
        {
...
bool acExists(const char* safName)
{
  ClRcT rc;
  ...
  rc = clAmsMgmtEntityGetConfig(mgmtHandle,&sg,&pEntityConfig);
  ...
  if ((CL_GET_ERROR_CODE(rc) == CL_ERR_DOESNT_EXIST)||(CL_GET_ERROR_CODE(rc) == CL_ERR_NOT_EXIST))
    {
      return false;
    }
  checkError("app container exists",rc);
  return false; // should never get here checkError will throw
}
...
void acRetract(const char* appCnt, const char* nodeNames[], int numNodes,const char* nameModifier)
{
  ...
  if ((rc = clAmsMgmtCCBCommit(ccbHandle) ) != CL_OK) checkError("Committing deletion of COMP/SU", rc);
    ...
    // application running.
void acAddWork(const char* appCnt,const char* workName, const char* activeXml, const char* standbyXml)
{
  ClRcT rc;
  ClAmsEntityConfigT sg;
  ...
  if (ccbHandle==CL_HANDLE_INVALID_VALUE) initHandles();
  initEntity(&sg,appCnt,CL_AMS_ENTITY_TYPE_SG);
  initEntity(&si,workName,CL_AMS_ENTITY_TYPE_SI);
  initEntity(&csi,workName,CL_AMS_ENTITY_TYPE_CSI);
  ...
  if (1) // configure the CSI
    {
      ...
      // ??? TODO: issue on marshall/unmarshall if not define
      saNameCopy(&nvp.csiName,&csi.name);
    ...
    // application
void acRemoveWork(const char* appCnt, const char* workName)
{
  ClRcT rc;
  //ClAmsEntityConfigT sg;
  ClAmsEntityConfigT si;
               ...
               const char *activecfg, const char *standbycfg)
{
    ...
    acAddWork(app, wrk, activecfg, standbycfg);
}

// removes su/comp, si/csi (work) and sg
void removeApp(const char *appName, const char *nodenames[], int numNodes)
{
    char wrk[CL_MAX_NAME_LENGTH];
    wrk[0] = 0;
    strncat(wrk, appName, sizeof(wrk)-1);
    strncat(wrk, "_Wrk", sizeof(wrk)-1);
    // delete su/comp on the nodes
    acRetract(appName, nodenames, numNodes);
    // remove si/csi
    acRemoveWork(appName, wrk);
    //remove sg itself
    removeAppCnt(appName);
}
