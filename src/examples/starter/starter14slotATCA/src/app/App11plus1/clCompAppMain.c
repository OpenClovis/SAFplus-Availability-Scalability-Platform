...
int main(int argc, char *argv[])
{
    ...
    rc = clApp11plus1_EOClientInstall();
...
ClRcT GetVaiFromCsi(const SaNameT       *compName,const char* name, VirtualIpAddress* ret)
{
    ...
    else
    {
        ...
        for(i = 0; i < csiRef.numCSIs; ++i)
        {
            ...
            if (strncmp(pCSI->csiDescriptor.csiName.value,name, pCSI->csiDescriptor.csiName.length) ==0)
            {                        
...
//                         STRING_HA_STATE((ClUint32T)pCSI->haState)));
                ...
                GetVirtualAddressInfo((SaAmfCSIDescriptorT*)&pCSI->csiDescriptor, ret);
...
{    
    ...
    else
    {
        for(i = 0; i < csiRef.numCSIs; ++i)
        {
            ...
            GetVirtualAddressInfo((SaAmfCSIDescriptorT*)&pCSI->csiDescriptor, &temp);
...
void HandleCsiChange(char* op,const SaNameT* compName, SaAmfCSIDescriptorT* csiDescriptor)
{
            ...
            {
                VirtualIpAddress temp;
                if (CL_OK == GetVaiFromCsi(compName,(char*)(&(csiDescriptor->csiName.value)), &temp))
                	AddRemVirtualAddress(op,&temp);
            }        
