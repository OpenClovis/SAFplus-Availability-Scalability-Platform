...
from clPythonBindings import clTimerApi, clCommonErrors

timerHandle = clTimerApi.ClTimerHandleT()
myCallback = clTimerApi.ClTimerCallBackT()
msg = "Hello world"
pData = None
...
def main():
    ...
    rc = clTimerApi.clTimerDelete(clTimerApi)
    if rc != clCommonErrors.CL_OK:
        clprintf(eClLogSeverityT.CL_LOG_SEV_ERROR, "Timer delete error[0x%X]", rc)
...
def timerCreate():
    ...
    timeout = clTimerApi.ClTimerTimeOutT(tsSec = 2, tsMilliSec = 0)
    pData = clUtils.toCharP(msg)
    myCallback = clTimerApi.ClTimerCallBackT(timerCallback)

    rc = clTimerApi.clTimerCreateAndStart(
        timeout,
        clTimerApi.eClTimerTypeT.CL_TIMER_REPETITIVE,
        clTimerApi.eClTimerContextT.CL_TIMER_SEPARATE_CONTEXT,
        myCallback,
        pData,
        timerHandle
        )
    if rc != clCommonErrors.CL_OK:
        clprintf(
            eClLogSeverityT.CL_LOG_SEV_ERROR,
            "timer created failure with rc [0x%x]",
            rc
        )
    else:
        clprintf(
            eClLogSeverityT.CL_LOG_SEV_INFO,
            "timer created sucessfully with handle [0x%x]",
            timerHandle
        )

def timerCallback(arg):
    pStr = ctypes.cast(arg, ctypes.c_char_p)
    clprintf(
            eClLogSeverityT.CL_LOG_SEV_INFO,
            "Timer callback called. msg: %s",
            pStr
        )
