...
extern ClRcT alarm_clock_EOClientInstall(void);
...
/* Keeps track of previous haState
 * so as to know when state change from
 * standby to active has occurred
 */
#define SA_AMF_HA_NONE 0
static SaAmfHAStateT   prevHaState = SA_AMF_HA_NONE;

/* checkpoint handle for alarm clocks
 */
static ClBoolT  alarmClockCkptOpened = CL_FALSE;
static ClCharT  alarmClockCkptName[CL_MAX_NAME_LENGTH];
ClCkptHdlT      alarmClockCkptHdl = 0;

/* is the checkpoint hot standby
 */
ClBoolT         alarmClockCkptHot = CL_FALSE;
...
int main(int argc, char *argv[])
{
    ...
    rc = alarm_clock_EOClientInstall();
                        ...
                        SaAmfCSIDescriptorT csiDescriptor)
{
    ...
    /* create a checkpoint based on the csi descriptor name 
     * to ensure uniqueness
     */
    if (alarmClockCkptOpened != CL_TRUE)
    {
        strcpy(alarmClockCkptName, (char *)csiDescriptor.csiName.value); 

        /* hard-coded to create a hot-standby checkpoint; however
         * CSI descriptor can have this information to make it
         * runtime configurable; else command line arguments can
         * also specify this value
         */
        alarmClockCkptHot = CL_TRUE;

        rc = alarmClockCkptCreate(alarmClockCkptName, 1, sizeof(acClockT), 
                                  &alarmClockCkptHdl, &alarmClockCkptHot);
        if (rc != CL_OK)
        {
            alarmClockCkptHdl = 0;
        }
    ...
    case SA_AMF_HA_ACTIVE:
        {
            ...
            if (rc != CL_OK)
            {
                alarmClockLogWrite(CL_LOG_SEV_ERROR, 
                                   "alarmClockCkpt: Failed to activate ckpt: 0x%x\n", rc);
            }
            ...
            if (rc == CL_OK)
            {
                ...
                rc = alarmClockCkptRead ( alarmClockCkptHdl, 1, 
                                          &alarmClock, sizeof(acClockT));
                if (rc == CL_OK)
                {
