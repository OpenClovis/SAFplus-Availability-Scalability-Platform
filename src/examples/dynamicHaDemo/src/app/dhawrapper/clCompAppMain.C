...
void* clDhaDemoCreate(void* arg)
{
    ...
    clDhaExmpExec(("Running MGMT initialize"),
                  ( rc = clAmsMgmtInitialize(&mgmtHandle, NULL, &version) ) == CL_OK,
                  ("MGMT initialize returned [%#x]", rc),
                  goto out);
...
out2:
    clDhaExmpExec(("Running CCB finalize"),
                  (rc = clAmsMgmtCCBFinalize(ccbHandle) ) == CL_OK,
                  ("CCB finalize returned [%#x]", rc),
                  goto out1);

out1:
    clDhaExmpExec(("Running MGMT finalize"),
                  (rc = clAmsMgmtFinalize(mgmtHandle)) == CL_OK,
...
out:
    ...
    {
        clprintf(CL_LOG_SEV_NOTICE,
                 "Successfully Created 2N SG [%sSG] and its entities(si, csi, su, comp, etc)",
                 pBaseName);
    }
...
out:
    ...
    {
        clprintf(CL_LOG_SEV_NOTICE,
                 "Successfully deleted SG [%sSG] and other entities(si, csi, su, comp, etc)",
                 pBaseName);
    }
