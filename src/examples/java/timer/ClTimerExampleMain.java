...
import clTimerApi.ClTimerLibrary.*;
import clTimerApi.ClTimerLibrary;

public class ClTimerExampleMain {
  ...
  private static clTimerApi.ClTimerLibrary timerLib = clTimerApi.ClTimerLibrary.INSTANCE;
  ...
  private static LongByReference timerHandle = new LongByReference();  
  ...
  public static void main(String argv[]) {
     ...
     */
    timerLib.clTimerDelete(timerHandle);
  ...
  */
  ...
  static int timerCreate() {    
    clTimerApi.ClTimerTimeOutT.ByValue timeout = new clTimerApi.ClTimerTimeOutT.ByValue();
    timeout.tsSec = 2;
    timeout.tsMilliSec = 0;
    timeout.write();
    Pointer arg = new Memory(4);
    arg.setInt(0,1999);
    int rc = timerLib.clTimerCreateAndStart(timeout, clTimerApi.ClTimerLibrary.ClTimerTypeT.CL_TIMER_REPETITIVE, clTimerApi.ClTimerLibrary.ClTimerContextT.CL_TIMER_SEPARATE_CONTEXT, timerCb, arg, timerHandle);
    if (rc == 0) {      
      clprintf(clUtils.ClUtilsLibrary.ClLogSeverityT.CL_LOG_SEV_INFO, String.format("timer created sucessfully with handle [0x%x]", timerHandle.getValue()));
    }
    else {
      clprintf(clUtils.ClUtilsLibrary.ClLogSeverityT.CL_LOG_SEV_ERROR, String.format("timer created failure with rc [0x%x]", rc));
    }
    return rc;
  }
  
  private static ClTimerCallBackT timerCb = (Pointer pActionArgument) -> {
    int rc = 0;
    clprintf(clUtils.ClUtilsLibrary.ClLogSeverityT.CL_LOG_SEV_INFO, String.format("inside timer callback: %d", pActionArgument.getInt(0)));
    return rc;
  };
}
