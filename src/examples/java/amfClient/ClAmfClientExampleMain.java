...
import clAmfClient.ClAmfClientLibrary;
import clAmfClient.ClAmfClientLibrary.*;
...
public class ClCompAppMain {
  ...
  private static clAmfClient.ClAmfClientLibrary amfClientLib = clAmfClient.ClAmfClientLibrary.INSTANCE;
  ...
  public static void main(String argv[]) {
     ...
     */
    clAmfClient.ClCpmSlotInfoT.ByReference slotInfo = new clAmfClient.ClCpmSlotInfoT.ByReference();
    slotInfo.slotId = 1;
    slotInfo.write();
    rc = amfClientLib.clCpmSlotInfoGet(ClCpmSlotInfoFieldIdT.CL_CPM_SLOT_ID, slotInfo);
    if (rc == 0) {
      slotInfo.read();
      slotInfo.nodeName.read();
      clprintf(clUtils.ClUtilsLibrary.ClLogSeverityT.CL_LOG_SEV_INFO,"clCpmSlotInfoGet: nodeAddr [%d], node name [%s]", slotInfo.nodeIocAddress, slotInfo.nodeName.toNameString());
    }
    else {
      clprintf(clUtils.ClUtilsLibrary.ClLogSeverityT.CL_LOG_SEV_ERROR,"clCpmSlotInfoGet error [0x%x]", rc);
    }
