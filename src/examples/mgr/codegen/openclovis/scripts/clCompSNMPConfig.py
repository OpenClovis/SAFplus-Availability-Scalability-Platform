...
import getpass
import shutil
from templates import clCompSNMPConfigTemplate
...
def processResourceAttributes(res, moString, result, depth, moName): # moName will not be same as name of res in case of Inheritence RelationShip
	...
	if isPmEnabled(res):
		pmList = res.getElementsByTagName("pm")
		serviceMap["serviceId"] = "CL_COR_SVC_ID_PM_MANAGEMENT"
		serviceMap["depth"] = depth
		serviceString = clCompSNMPConfigTemplate.serviceIdTemplate.safe_substitute(serviceMap)
		for pm in pmList:
			pmAttrList = pm.getElementsByTagName("attribute")
			result += processAttributeList(pmAttrList, moName, moString, serviceString)
	...
	pmList = res.getElementsByTagName("pm")
	if len(pmList) > 0:
		pmAttrList = pmList[0].getElementsByTagName("attribute")
	for moAttribute in moAttributeList:
		if moAttribute not in provAttrList and moAttribute not in pmAttrList:
...
def processAlarmList(alarmList, moName, moString, serviceString):
	...
	for alarm  in alarmList:
		...
		if alarmProfile != None:
			...
			specificProb = alarmProfile.attributes["SpecificProblem"].value
			try:
			...
			if alarmOid != "":
				...
				trapMap["probCause"] = alarmPC
				trapMap["specProb"] = specificProb 
				alarmEntryString += clCompSNMPConfigTemplate.probableCauseTemplate.safe_substitute(trapMap)
#------------------------------------------------------------------------------
#-----------------------------------------------------------------------
