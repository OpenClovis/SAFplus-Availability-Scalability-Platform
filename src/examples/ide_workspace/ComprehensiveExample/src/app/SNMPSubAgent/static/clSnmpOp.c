...
ClSNMPRequestInfoT gReqInfo ;
ClMedHdlPtrT gMedSnmpHdl;
        ...
        ClSnmpOpCodeT opCode)
{
    ...
    *my_data_context = &gReqInfo;
    ...
    memset(&reqInfo, 0, sizeof(ClSNMPRequestInfoT));
    ...
    if((opCode ==  CL_SNMP_GET_NEXT) || (my_loop_context !=  NULL && (*(ClInt32T*)*my_loop_context)!= 1))
    {
        ...
        {
            /*We are assuming that the ClSNMPRequestInfoT structure is
    ...
    switch (retVal)
    {
        ...
        default:                 /* Failed due to other reasons */
            clLogDebug("SNP", NULL, "Failed while getting next entry. rc[0x%x]", retVal);
            return retVal;
    }
    return CL_OK;
}

ClRcT clExecuteSnmp (ClSNMPRequestInfoT* reqInfo,
        void* data,
        ClInt32T *pOpNum,
        ClInt32T *pErrCode)
{
...
ClRcT clRequestAdd (ClSNMPRequestInfoT* reqInfo,
        void* data,
        ClInt32T *pOpNum,
        ClInt32T *pErrCode,
        ClPtrT   pReqAddInfo)
{
    ...
    switch (reqInfo->opCode)
    {
        ...
        case CL_SNMP_DELETE:
            ...
            break;
        default:
            clLogError("SNP", NULL, "Invalid request type.");
    }
    
    return rc;
}
ClUint32T clGetTable ( ClSNMPRequestInfoT* reqInfo,
        void* data, 
        ClInt32T* pErrCode)
{
    ClMedOpT        opInfo;
    ...
    if (tempVarInfo == NULL)
    {
        clLogError("SNM", NULL, "Failed while allocating the varbind.");
    ...
    errorCode = clMedOperationExecute (gMedSnmpHdl, &opInfo);
...
ClUint32T clGetTableAttr ( ClSNMPRequestInfoT* reqInfo,
        void *data, 
        ClInt32T *pErrCode)
{
    ClMedOpT opInfo;
...
/*
ClUint32T clSetTableAttr(  ClSNMPRequestInfoT* reqInfo,
        void* data,
        ClInt32T *pOpNum,
        ClInt32T *pErrCode)
{
    ...
    if(arrIndex < *pOpNum)
    {
        ...
        opInfo.varInfo[arrIndex].pInst = (ClSNMPRequestInfoT *)clHeapCalloc(1,sizeof(ClSNMPRequestInfoT) );
        if(NULL == opInfo.varInfo[arrIndex].pInst)
        {
            CL_DEBUG_PRINT(CL_DEBUG_ERROR, ("Could not allocate memory. Size asked for %u", (ClUint32T)sizeof(ClSNMPRequestInfoT) ) );
            return CL_SNMP_ERR_NOCREATION;
        }
        memcpy(opInfo.varInfo[arrIndex].pInst, reqInfo, sizeof(ClSNMPRequestInfoT) );
        ...
        ClPtrT pReqAddInfo)
{
    ...
    gOper.opInfo.varInfo[arrIndex].pInst = (ClSNMPRequestInfoT *)clHeapCalloc(1,sizeof(ClSNMPRequestInfoT) );
    if(NULL == gOper.opInfo.varInfo[arrIndex].pInst)
    {
        clLogError("SNP", NULL, 
                "Could not allocate memory. Size asked for [%u]", 
                (ClUint32T)sizeof(ClSNMPRequestInfoT) );
        clSnmpMutexUnlock(gOper.mtx);
        return CL_SNMP_ERR_NOCREATION;
    }
    memcpy(gOper.opInfo.varInfo[arrIndex].pInst, reqInfo, sizeof(ClSNMPRequestInfoT) );
    ...
    {
        rc = clMedInstValidate(gMedSnmpHdl, &gOper.opInfo.varInfo[arrIndex]);
        if(CL_OK != rc)
        {
            clLogError("SNM", NULL,
        ...
        {
            clLogDebug("SNM", NULL, "Adding the get job.");
...
{
    ClMedOpT    opInfo = {0};
    ClInt32T    errorCode = CL_OK, count = 0;
...
{
    clLogDebug("SNP", NULL, "Reseting the static data structure.");
    gOper.refCount = 1;
    clSnmpUndo();
}
        ...
        netsnmp_request_info              *requests)
{
    ...
    clLogDebug("SNM", NULL, "Inside MODE-END callback [%s] ...mode [%d]", 
            __FUNCTION__, reqinfo->mode );
    
        switch (reqinfo->mode)
    {
        case MODE_GETBULK:
            clLogDebug("SNM", NULL, "Inside mode BULK-GET");
            break;
        case MODE_GETNEXT:
        case MODE_GET:
            {
                ...
                clLogDebug("SNM", NULL, "Inside mode [%s] vbC[%d] noOfOps [%d]",
                ...
                if (gErrorHappened == CL_TRUE)
                {
                    ...
                    clLogNotice("SNM", NULL, "Error happened in the validating the request.");
                ...
                {
                    clLogDebug("SNP", NULL, "Sending the get request now as the vbCount and varCount are equal.");
                    rc = clSnmpProcessRequest(requests);
                    if (CL_OK != rc)
                        clLogError("SNP", NULL, "Failed while processing the GET request. rc[0x%x]", rc);
  		            noOfOps = 0;
                }
            }
            break;
        case MODE_SET_RESERVE1:
            clLogDebug("SNM", NULL, "Inside mode RESERVE1");
            break;
        case MODE_SET_RESERVE2:
            clLogDebug("SNM", NULL, "Inside mode RESERVE2");
            break;
        case MODE_SET_ACTION:
            clLogDebug("SNM", NULL, "Inside mode ACTION");
            break;
        case MODE_SET_COMMIT:
            clLogDebug("SNM", NULL, "Inside mode COMMIT");
            break;
        case MODE_SET_UNDO:
            clLogDebug("SNM", NULL, "Inside mode UNDO");
            break;
        case MODE_SET_FREE:
            clLogDebug("SNM", NULL, "Inside mode FREE");
            break;
        default:
            clLogDebug("SNM", NULL, "Inside default case");
    }
