...
/*  Do the CPM client init/Register */
    ...
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Application Address 0x%x Port %x\n", clIocLocalAddressGet(), iocPort));
    rc = saAmfInitialize(&amfHandle, &callbacks, &version);
    if(SA_AIS_OK != rc)
    {
        clLogError("SNP", NULL,
                "Amf intialization failed, rc=[0x%x]", rc);
        return -1;
    }
    ...
    {
        clLogError("SNP", NULL,
                "saAmfSelectionObjectGet() failed, rc=[0x%x]", rc);
        return -1;
    }
    ...
    {
        clLogError("SNP", NULL,
                "Component name get failed, rc=[0x%x]", rc);
        return -1;
    }
    ...
    {
        clLogError("SNP", NULL,
                "Component register failed, rc=[0x%x]", rc);
        return -1;
    }
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("After clCpmClientRegister %x\n", rc));
    ...
    do
    {
        ...
        {
            clLogError("SNP", NULL, "Error in select()");
            perror("");
            break;
        }
    ...
    {
        clLogError("SNP", NULL, "AMF finalization error[0x%X]", rc);
    }

    clLogInfo("SNP", NULL, "AMF Finalized");
                        ...
                        const SaNameT  *compName)
{
    ...
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Inside appTerminate \n"));
                                                                                                                             
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Unregister with CPM before Exit ................. %s\n", compName->value));
    rc = saAmfComponentUnregister(amfHandle, compName, NULL);
    CL_DEBUG_PRINT(CL_DEBUG_TRACE, ("Finalize before Exit ................. %s\n", compName->value));
        ...
        SaAmfCSIDescriptorT   csiDescriptor)
{
    ...
    if(haState == SA_AMF_HA_QUIESCING)
    {
        ...
        clOsalPrintf("######## before saAmfCSIQuiescingComplete ########\n");
        saAmfCSIQuiescingComplete(amfHandle, invocation, SA_AIS_OK);
    }
    else
    {
        clOsalPrintf("######## before saAmfResponse ########\n");
